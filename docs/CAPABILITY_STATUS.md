...
| `TLALOC.EVIDENCE.RUN_RECORD` | Emit immutable model-run records with a canonical environment hash, monthly index, and executable replay that reproduces deterministic output. | `implemented` | `test:behavior-lab/internal/runrecord:TestEnvHashExcludesOnlyIdentityOutcomeAndVariableAxis`<br>`test:behavior-lab/internal/runrecord:TestStoreIsImmutableAndIndexesRun`<br>`test:behavior-lab/internal/runrecord:TestReplayFromRecordReproducesEnvAndOutputHashes`<br>`test:behavior-lab/internal/realcampaign:TestDoctorAndPrepareDiscoverOneRealModel` | `unreleased` | 2026-09-01 | The first active integration records the deterministic realcampaign prepare path; trace remains empty until the trajectory phase. |
