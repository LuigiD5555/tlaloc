...
- Bash syntax for `tools/tlaloc`, `tests/test-skills.sh`, `tests/test-project-skill-install.sh`, and `tests/test-independent-install.sh`;
- `tests/test-skills.sh`;
- `tests/test-project-skill-install.sh`;
- `tests/test-version-coherence.sh`;
- `tests/test-current-terminology.sh`;
- isolated managed install -> skill checks -> doctor -> uninstall;
- generated prompt hash must remain unchanged;
- downstream Tonal CI must fetch the promoted Tlaloc alpha.10 exact commit and pass Tlaloc Behavior Lab `go test ./...` and `go vet ./...` before Tonal updates its released lock.
...
Promote only after the Tlaloc impact-closure gates pass. Tonal composition promotion is a separate downstream gate.
