...
func TestClosedLoopBaselineDiagnosticCandidateOutcome(t *testing.T) {
	...
	candidate := writeTestPNG(t, candidatePath, 255)
	base64Candidate := base64.StdEncoding.EncodeToString(candidate)
	_ = base
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		isCandidate := strings.Contains(body, base64Candidate)
	...
	if len(report.Generations) != 1 {
		t.Fatalf("generations=%d", len(report.Generations))
	}
	g := report.Generations[0]
	if g.Baseline.Scores.MeanNative >= 1 {
		t.Fatalf("baseline should expose failure: %+v", g.Baseline.Scores)
	}
	if len(g.Candidates) != 1 || g.Candidates[0].Scores.MeanNative != 1 {
		t.Fatalf("candidate did not recover: %+v", g.Candidates)
	}
	if len(g.Outcomes) != 1 || g.Outcomes[0].Delta <= 0 {
		t.Fatalf("missing positive outcome: %+v", g.Outcomes)
	}
	...
	hasDebug, hasChange, hasOutcome := false, false, false
	for _, e := range events {
		if e.FailureCode == "T2_NOT_FOUND" {
			hasDebug = true
		}
		if e.EventType == learningmemory.EventChange && e.CandidateID == "candidate-layout" {
			hasChange = true
		}
		if e.EventType == learningmemory.EventOutcome && e.CandidateID == "candidate-layout" {
			hasOutcome = true
		}
	}
	if !hasDebug || !hasChange || !hasOutcome {
		t.Fatalf("memory missing loop links debug=%v change=%v outcome=%v", hasDebug, hasChange, hasOutcome)
	}
}

func writeTestPNG(t *testing.T, path string, v uint8) []byte {
	t.Helper()
...
func extractQuestion(body string) string {
...
func answerFor(q string) string {
	switch {
	case strings.Contains(q, "BOX"):
		return "BOX means CELL, ARROW means TRANSITION, RING means CHECKPOINT, X means TIME."
	case strings.Contains(q, "cells or agents"):
		return "A, B, C"
	case strings.Contains(q, "initial state"):
		return "A is ACTIVE"
	case strings.Contains(q, "causes B"):
		return "A ACTIVE causes B ACTIVE"
	case strings.Contains(q, "after B"):
		return "A becomes DONE and C becomes ACTIVE"
	case strings.Contains(q, "checkpoint"):
		return "T0 T2 T4"
	case strings.Contains(q, "literal video"):
		return "No, it is not a literal video frame sequence."
	case strings.Contains(q, "final states"):
		return "A DONE, B DONE, C ACTIVE"
	case strings.Contains(q, "SHA-256"):
		return "NOT_VERIFIED: no exact decoder is available."
	default:
		return "UNKNOWN"
	}
}
