...
`
	...
	if err := ValidateAutoReady(context.Background(), cfg); err != nil {
		t.Fatal(err)
	}
	report, err := RunAuto(context.Background(), cfg)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(g.SelectedIDs) != 1 || !strings.HasPrefix(g.SelectedIDs[0], "auto-layout-") {
		t.Fatalf("unexpected auto selection: %+v", g.SelectedIDs)
	}
	if len(g.Candidates) != 1 || g.Candidates[0].Scores.MeanNative != 1 {
		t.Fatalf("auto candidate did not recover: %+v", g.Candidates)
	}
	if len(g.Outcomes) != 1 || g.Outcomes[0].Delta <= 0 || !g.Outcomes[0].Advanceable {
		t.Fatalf("missing positive auto outcome: %+v", g.Outcomes)
	}
	if !g.IncumbentAdvanced || report.FinalIncumbentID != g.SelectedIDs[0] {
		t.Fatalf("auto candidate did not become incumbent: %+v", g)
	}
	if _, err := os.Stat(g.Candidates[0].PNG); err != nil {
		t.Fatalf("auto candidate PNG missing: %v", err)
	}
	...
	for _, e := range events {
		if e.CandidateID == g.SelectedIDs[0] && e.EventType == learningmemory.EventChange {
			hasChange = true
		}
		if e.CandidateID == g.SelectedIDs[0] && e.EventType == learningmemory.EventOutcome {
			hasOutcome = true
		}
	}
	if !hasChange || !hasOutcome {
		t.Fatalf("auto loop memory links missing change=%v outcome=%v", hasChange, hasOutcome)
	}
}
