...
func TestAutoCandidateConfigsAreDeterministicAndCapabilityFiltered(t *testing.T) {
	...
	f, err := os.Create(parentPNG)
	if err != nil {
		t.Fatal(err)
	}
	if err := png.Encode(f, image.NewGray(image.Rect(0, 0, 640, 640))); err != nil {
		t.Fatal(err)
	}
	...
	first, err := p.autoCandidateConfigs(plan, caps, parent, filepath.Join(tmp, "g1"))
	if err != nil {
		t.Fatal(err)
	}
	second, err := p.autoCandidateConfigs(plan, caps, parent, filepath.Join(tmp, "g1"))
	if err != nil {
		t.Fatal(err)
	}
	if len(first) != 2 || len(second) != 2 {
		t.Fatalf("expected two supported candidates, got %d/%d", len(first), len(second))
	}
	for i := range first {
		if first[i].ID != second[i].ID {
			t.Fatalf("non-deterministic id: %q != %q", first[i].ID, second[i].ID)
		}
		if first[i].ParentSpecimenID != parent.ID {
			t.Fatalf("wrong parent: %#v", first[i])
		}
		if len(first[i].Mutations) != 1 {
			t.Fatalf("auto candidates must isolate one mutation: %#v", first[i])
		}
		if len(first[i].BuildCommand) != 1 || first[i].BuildCommand[0] != "origami-candidate-build" {
			t.Fatalf("wrong build hook: %#v", first[i])
		}
		if first[i].Mutations[0].Kind == visualsearch.MutationDepthStructure {
			t.Fatal("unsupported mutation leaked into candidate bank")
		}
	}
}
...
`
	...
	caps, err := validateAutoConfig(context.Background(), p)
	if err != nil {
		t.Fatal(err)
	}
	if caps.ExactPlaneMutation || len(caps.SupportedKinds) != 1 || caps.SupportedKinds[0] != "LAYOUT" {
...
`
	...
	p := prepared{cfg: Config{AutoCandidates: true, CandidateBuilder: []string{builder}, AutoCandidateBaseProfileID: "origami.temporal-carrier.r0.profile-1"}}
	if _, err := validateAutoConfig(context.Background(), p); err == nil {
		t.Fatal("expected exact-plane builder rejection")
	}
}
