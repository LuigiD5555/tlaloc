...
func TestPrepareRunsBuilderAndAllPreVLMGates(t *testing.T) {
	d := t.TempDir()
	parent := filepath.Join(d, "parent.png")
...
JSON
`
	if err := os.WriteFile(builder, []byte(script), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	r, err := Prepare(Request{Builder: builder, ParentPNG: parent, Candidate: c, OutputDir: filepath.Join(d, "out")})
	if err != nil {
		t.Fatal(err)
	}
	if !r.EligibleForVLM || !r.BuildValid || !r.Parity.Pass || !r.VisibleTextFidelity.Pass || !r.RegressionPrecheck.Pass {
		t.Fatalf("report=%#v", r)
	}
	if _, err := os.Stat(r.VisibleTextManifest); err != nil {
		t.Fatalf("visible text artifact missing: %v", err)
	}
}
