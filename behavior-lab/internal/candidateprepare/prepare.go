...
func Prepare(req Request) (Report, error) {
	if req.Builder == "" {
		req.Builder = "origami-candidate-build"
	}
	if req.ParentPNG == "" || req.Candidate.ID == "" {
		return Report{}, fmt.Errorf("parent PNG and candidate are required")
	}
	if req.OutputDir == "" {
		req.OutputDir = filepath.Join(".", "tlaloc-candidates", req.Candidate.ID)
	}
	if err := os.MkdirAll(req.OutputDir, 0o700); err != nil {
		return Report{}, err
	}
	parentBody, err := os.ReadFile(req.ParentPNG)
	if err != nil {
		return Report{}, err
	}
	sum := sha256.Sum256(parentBody)
	parentSHA := hex.EncodeToString(sum[:])
	origamiSpec, err := experimentpolicy.ToOrigamiSpec(req.Candidate, parentSHA)
	if err != nil {
		return Report{}, err
	}
	...
	if err := writeJSON(specPath, origamiSpec); err != nil {
		return Report{}, err
	}
	inherited, err := json.Marshal(req.InheritedMutations)
	if err != nil {
		return Report{}, err
	}
	if err := run(req.Builder, "semantic-manifest", "-in", req.ParentPNG, "-mutations-json", string(inherited), "-out", expectedPath); err != nil {
		return Report{}, fmt.Errorf("expected semantic manifest: %w", err)
	}
	if err := run(req.Builder, "build", "-parent", req.ParentPNG, "-out", pngPath, "-spec", specPath, "-inherited-mutations-json", string(inherited), "-interop-report", buildPath); err != nil {
		return Report{}, fmt.Errorf("candidate build: %w", err)
	}
	var expected experimentpolicy.SemanticManifest
	if err := readJSON(expectedPath, &expected); err != nil {
		return Report{}, err
	}
	var build experimentpolicy.BuildManifest
	if err := readJSON(buildPath, &build); err != nil {
		return Report{}, err
	}
	...
	if err := writeJSON(visibleTextPath, build.VisibleText); err != nil {
		return Report{}, err
	}
	...
	if err := writeJSON(filepath.Join(req.OutputDir, "preparation-report.json"), report); err != nil {
		return Report{}, err
	}
	if !report.Parity.Pass {
		return report, fmt.Errorf("semantic parity failed: %s", report.Parity.FailureCode)
	}
	if !report.VisibleTextFidelity.Pass {
		return report, fmt.Errorf("visible text fidelity failed: %s", report.VisibleTextFidelity.FailureCode)
	}
	if !report.RegressionPrecheck.Pass {
		return report, fmt.Errorf("regression precheck failed")
	}
	return report, nil
}
