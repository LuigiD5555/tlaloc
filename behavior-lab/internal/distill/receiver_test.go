...
func TestDistillCollapsesRepeatedDeterministicSteps(t *testing.T) {
	...
	if c.Schema != ContractID { t.Fatalf("unexpected schema %q", c.Schema) }
	if len(c.Program) != 3 { t.Fatalf("expected 3 unique rules, got %d", len(c.Program)) }
	if c.SourceTraceSHA256 == "" || c.ID == "" { t.Fatal("missing provenance identity") }
}
...
func TestTournamentRejectsFalseExactAndContamination(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if winner.Candidate.ID != "good" { t.Fatalf("unexpected winner %q", winner.Candidate.ID) }
}
