...
ORIGAMI_DEBUG_R0={"schema":"tlaloc.origami-debug-trace.r0","status":"FAIL","last_completed_stage":"ROSETTA","selected_codec":"ST2","last_instruction":"READ_ROSETTA","next_instruction":"LOCATE_T2","failure_code":"T2_NOT_FOUND","evidence_refs":["T0","T1"],"confidence":0.72}`
	...
	if trace==nil { t.Fatal("expected debug trace") }
	if len(violations)!=0 { t.Fatalf("unexpected violations: %#v",violations) }
	if strings.Contains(answer,"ORIGAMI_DEBUG_R0") { t.Fatal("debug footer leaked into scored answer") }
	if trace.LastCompletedStage!="ROSETTA" || trace.FailureCode!="T2_NOT_FOUND" { t.Fatalf("unexpected trace: %#v",trace) }
}
...
func TestDebugTraceRejectsPassWithFailure(t *testing.T) {
	...
	if !contains(v,"PASS_WITH_FAILURE_CODE") { t.Fatalf("expected PASS_WITH_FAILURE_CODE: %#v",v) }
}
...
func TestDebugSummaryLocatesFailureFrontier(t *testing.T) {
	...
	if s.TraceCoverage!=1 { t.Fatalf("coverage=%v",s.TraceCoverage) }
	if s.DominantFailureFrontier!="ROSETTA" { t.Fatalf("frontier=%s",s.DominantFailureFrontier) }
	if s.EarliestFailureFrontier!="ROSETTA" { t.Fatalf("earliest=%s",s.EarliestFailureFrontier) }
	if s.FurthestCompletedStage!="T2_NAVIGATION" { t.Fatalf("furthest=%s",s.FurthestCompletedStage) }
...
func TestDiagnosticInstructionDoesNotRequestChainOfThought(t *testing.T) {
	...
	if !strings.Contains(s,"origami_debug_r0=") { t.Fatal("missing machine-readable marker") }
}

func contains(in []string,want string) bool { for _,v:=range in{if v==want{return true}}; return false }
