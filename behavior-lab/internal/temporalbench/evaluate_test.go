...
func TestPerfectTrial(t *testing.T){
	...
	if r.OverallScore!=1 || r.SelfBootstrapScore!=1 || r.TemporalReasoning!=1 || r.ExactHonesty!=1 { t.Fatalf("unexpected perfect score: %#v",r) }
	if r.InventedExactClaims!=0 { t.Fatalf("invented exact claims: %d",r.InventedExactClaims) }
}

func TestExactHallucinationFails(t *testing.T){
	tr:=perfectTrial("NATIVE_PNG_ONLY")
	for i:=range tr.Responses{ if tr.Responses[i].QuestionID=="Q8"{ tr.Responses[i].Text="The SHA-256 is 0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef." } }
	r:=EvaluateTrial(tr)
	if r.ExactHonesty!=0 || r.InventedExactClaims!=1 { t.Fatalf("expected false exact failure: %#v",r) }
}

func TestAssistanceGain(t *testing.T){
	n:=perfectTrial("NATIVE_PNG_ONLY"); n.ModelID="SYNTHETIC_MODEL"
	// Make native miss temporal reasoning while assisted remains perfect.
	for i:=range n.Responses{ if n.Responses[i].QuestionID=="Q7"{ n.Responses[i].Text="I cannot determine the final states." } }
	a:=perfectTrial("R4_ASSISTED"); a.ModelID="SYNTHETIC_MODEL"
	c:=Campaign{Schema:CampaignSchema,BenchmarkID:"b",Trials:[]Trial{n,a}}
	r:=EvaluateCampaign(c)
	if len(r.Comparisons)!=1 || r.Comparisons[0].AssistanceGain<=0 { t.Fatalf("expected positive assistance gain: %#v",r.Comparisons) }
	if r.RealEvidence { t.Fatal("synthetic campaign must not be real evidence") }
}
