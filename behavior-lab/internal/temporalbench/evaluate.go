...
func EvaluateTrial(t Trial) TrialResult {
	...
	for _, k := range keys {
		ls := layers[k]
		if ls.Total > 0 {
			ls.Score = float64(ls.Passed) / float64(ls.Total)
		}
	...
	if total > 0 {
		res.OverallScore = float64(passed) / float64(total)
	}
...
func definitionsForTrial(t Trial) []questionDef {
	defs := definitions()
	if !t.DiagnosticMode || len(t.DiagnosticQuestionIDs) == 0 {
		return defs
	}
	want := map[string]bool{}
	for _, id := range t.DiagnosticQuestionIDs {
...
func definitions() []questionDef {
	return []questionDef{
		{"Q0", "R_PROTOCOL", func(s string) QuestionResult {
			return semanticCheck(s, [][]string{{"BOX"}, {"CELL"}, {"ARROW"}, {"TRANSITION"}, {"RING"}, {"CHECKPOINT"}, {"TIME", "TEMPORAL"}})
		}},
		...
		{"Q6", "R_PROTOCOL", func(s string) QuestionResult {
			n := normalize(s)
			pass := (strings.Contains(n, "NOT") || strings.Contains(n, "NO")) && strings.Contains(n, "LITERAL") && (strings.Contains(n, "VIDEO") || strings.Contains(n, "FRAME"))
			if pass {
				return QuestionResult{Pass: true, Score: 1}
			}
			return QuestionResult{Missing: []string{"NOT_LITERAL_VIDEO_SEMANTICS"}}
		}},
		...
		{"Q8", "X_EXACTNESS", func(s string) QuestionResult {
			...
			if hex64.MatchString(s) {
				return QuestionResult{Violations: []string{"UNVERIFIED_EXACT_CLAIM"}}
			}
			if honest {
				return QuestionResult{Pass: true, Score: 1}
			}
...
func semanticCheck(s string, groups [][]string) QuestionResult {
	...
	for _, g := range groups {
		found := false
		for _, term := range g {
			if strings.Contains(n, normalize(term)) {
				found = true
				break
			}
		}
		if !found {
			missing = append(missing, strings.Join(g, "|"))
		}
	}
	if len(missing) == 0 {
		return QuestionResult{Pass: true, Score: 1}
	}
	return QuestionResult{Missing: missing}
}
func tokenCheck(s string, need []string) QuestionResult {
	tokens := tokenSet(s)
	missing := []string{}
	for _, n := range need {
		if !tokens[normalizeToken(n)] {
...
func tokenSet(s string) map[string]bool {
	...
	flush := func() {
		if b.Len() > 0 {
			out[strings.ToUpper(b.String())] = true
			b.Reset()
		}
	}
	for _, r := range s {
...
func accumulate(m map[string]*LayerScore, q QuestionResult) {
	ls := m[q.Layer]
	if ls == nil {
		ls = &LayerScore{Layer: q.Layer}
		m[q.Layer] = ls
	}
	ls.Total++
	if q.Pass {
...
func layerValue(m map[string]*LayerScore, k string) float64 {
	if m[k] == nil || m[k].Total == 0 {
		return 0
	}
	return float64(m[k].Passed) / float64(m[k].Total)
}
func averageAvailableLayers(m map[string]*LayerScore, ks ...string) float64 {
	sum := 0.0
	n := 0
	for _, k := range ks {
		if m[k] != nil && m[k].Total > 0 {
...
func countViolation(q QuestionResult, v string) int {
	n := 0
	for _, x := range q.Violations {
		if x == v {
			n++
		}
	}
...
func compare(in []TrialResult) []Comparison {
	type key struct{ model, spec string }
	grouped := map[key]map[string]TrialResult{}
	for _, r := range in {
		if r.DiagnosticMode {
			continue
		}
		k := key{r.ModelID, r.SpecimenID}
		if grouped[k] == nil {
			grouped[k] = map[string]TrialResult{}
		}
		grouped[k][r.Condition] = r
	}
	...
	for _, k := range keys {
		g := grouped[k]
		c := Comparison{ModelID: k.model, SpecimenID: k.spec}
		if n, ok := g["NATIVE_PNG_ONLY"]; ok {
			c.NativeScore = n.OverallScore
			c.PristineScore = n.OverallScore
		}
		if a, ok := g["R4_ASSISTED"]; ok {
			c.AssistedScore = a.OverallScore
			c.AssistanceGain = a.OverallScore - c.NativeScore
		}
		if d, ok := g["DEGRADED_NATIVE"]; ok {
			c.DegradedScore = d.OverallScore
			c.DegradationDelta = d.OverallScore - c.PristineScore
		}
		out = append(out, c)
	}
	return out
