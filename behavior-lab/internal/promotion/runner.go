...
func ObservationQuestion(includeT3 bool) string {
	t3 := "Set t3 to null."
	if includeT3 {
		t3 = "If you can read T3 natively from the image, include carrier_id, store_root_sha256, source_sha256, page_count, block_count, document_count and object_count. Otherwise set t3 to null."
	}
...
func RunPerceptionTrial(ctx context.Context, client target.OpenAICompat, modelName string, trial int, variant TransportVariant, systemPrompt string, includeT3 bool) (TrialExecution, error) {
	result, err := client.CompletePerception(ctx, target.PerceptionInput{SystemPrompt: systemPrompt, Question: ObservationQuestion(includeT3), Image: variant.Bytes, MediaType: variant.MediaType})
	if err != nil {
		return TrialExecution{}, err
	}
	obs, err := ParseObservation(result.Content)
	if err != nil {
		return TrialExecution{}, fmt.Errorf("parse model observation: %w", err)
	}
	obs.Schema = "origami.perception-observation.r1"
	obs.Model = modelName
	obs.Trial = trial
	obs.Transport = variant.Name
	obs.EvidenceKind = "REAL_MODEL"
...
func ParseObservation(content string) (OrigamiObservation, error) {
	...
	if start < 0 || end < start {
		return OrigamiObservation{}, fmt.Errorf("no JSON object in model output")
	}
	...
	if err := dec.Decode(&raw); err != nil {
		return OrigamiObservation{}, err
	}
	var t3 any
	if len(raw.T3) > 0 && string(raw.T3) != "null" {
		if err := json.Unmarshal(raw.T3, &t3); err != nil {
			return OrigamiObservation{}, err
		}
	}
...
type OrigamiEvaluator struct {
	Binary  string
	Carrier string
}

func (e OrigamiEvaluator) Evaluate(ctx context.Context, observation OrigamiObservation) (EvaluatorReport, error) {
	if e.Binary == "" || e.Carrier == "" {
		return EvaluatorReport{}, fmt.Errorf("Origami evaluator binary and carrier are required")
	}
	body, err := json.Marshal(observation)
	if err != nil {
		return EvaluatorReport{}, err
	}
	tmp, err := os.CreateTemp("", "origami-observation-*.json")
	if err != nil {
		return EvaluatorReport{}, err
	}
	path := tmp.Name()
	...
	cmd := exec.CommandContext(ctx, e.Binary, "-carrier", e.Carrier, "-observation", path, "-out", "-")
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return EvaluatorReport{}, fmt.Errorf("origami evaluator: %w: %s", err, strings.TrimSpace(stderr.String()))
	}
	var report EvaluatorReport
	if err := json.Unmarshal(stdout.Bytes(), &report); err != nil {
		return EvaluatorReport{}, fmt.Errorf("decode evaluator report: %w", err)
	}
	return report, nil
}
