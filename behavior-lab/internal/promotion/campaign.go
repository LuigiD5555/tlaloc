...
type Gate struct { Name string `json:"name"`; Pass bool `json:"pass"`; Reason string `json:"reason,omitempty"` }
...
func Evaluate(records []TrialRecord, routing RoutingEvidence, policy Policy) (CampaignReport,error) {
	...
	for _,record:=range records{
		if record.Model==""||record.Trial<=0{return CampaignReport{},fmt.Errorf("invalid trial identity")}
		if record.Evaluation.Model!=""&&record.Evaluation.Model!=record.Model{return CampaignReport{},fmt.Errorf("evaluator/model mismatch for %s trial %d",record.Model,record.Trial)}
		s:=byModel[record.Model];if s==nil{s=&ModelSummary{Model:record.Model};byModel[record.Model]=s}
		real:=record.EvidenceKind=="REAL_MODEL"&&record.Evaluation.EvidenceKind=="REAL_MODEL"
		if real&&record.Evaluation.HybridTrialPromotionOK{transports[record.Transport]++;if record.Transport=="original"{s.RealOriginalHybridTrials++}}
		if real&&record.Transport=="original"&&record.Evaluation.NativeT3TrialPromotionOK{s.RealOriginalNativeT3Trials++}
		if real&&record.ToolLoopPassed&&record.ToolCalls>0&&record.AnswerPresent{s.RealToolLoopPasses++}
	}
	models:=make([]ModelSummary,0,len(byModel));for _,s:=range byModel{models=append(models,*s)};sort.Slice(models,func(i,j int)bool{return models[i].Model<models[j].Model})
	qualifiedHybrid:=0;qualifiedNative:=0;toolModels:=0
	for _,s:=range models{if s.RealOriginalHybridTrials>=policy.TrialsPerModel{qualifiedHybrid++};if s.RealOriginalNativeT3Trials>=policy.TrialsPerModel{qualifiedNative++};if s.RealToolLoopPasses>0{toolModels++}}
	crossHybrid:=qualifiedHybrid>=policy.MinModels
	transportPass:=true;for _,name:=range policy.RequiredTransports{if transports[name]<1{transportPass=false}}
...
func normalize(p Policy)Policy{d:=DefaultPolicy();if p.MinModels<=0{p.MinModels=d.MinModels};if p.TrialsPerModel<=0{p.TrialsPerModel=d.TrialsPerModel};if len(p.RequiredTransports)==0{p.RequiredTransports=d.RequiredTransports};if p.MinRoutingDocuments<=0{p.MinRoutingDocuments=d.MinRoutingDocuments};if p.MinRoutingQueries<=0{p.MinRoutingQueries=d.MinRoutingQueries};if p.MinPrimaryDocHitRate<=0{p.MinPrimaryDocHitRate=d.MinPrimaryDocHitRate};if p.MinVerifiedEvidenceRate<=0{p.MinVerifiedEvidenceRate=d.MinVerifiedEvidenceRate};return p}
func gateReason(pass bool,reason string)string{if pass{return ""};return reason}
