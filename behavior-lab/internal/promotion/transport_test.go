...
func TestBuildTransportVariants(t *testing.T) {
	variants, err := BuildTransportVariants(testPNG(t))
	if err != nil {
		t.Fatal(err)
	}
	if len(variants) != 4 {
	...
	for _, v := range variants {
		...
		if v.Width != size[0] || v.Height != size[1] || len(v.Bytes) == 0 {
			t.Fatalf("bad variant %+v", v)
		}
		if v.Name == "jpeg-preview" && v.MediaType != "image/jpeg" {
			t.Fatal("preview must be JPEG")
		}
	}
}
