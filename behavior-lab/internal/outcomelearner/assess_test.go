...
func TestInvalidSpecimenDoesNotPenalizeModel(t *testing.T){
	...
	if a.Classification!=OutcomeInvalidExperiment{t.Fatalf("class=%q",a.Classification)}
	if a.ModelPenaltyAllowed{t.Fatal("model penalty must be disabled")}
	if k.Action!="RETAIN_HYPOTHESIS_REJECT_SPECIMEN"{t.Fatalf("action=%q",k.Action)}
}
...
func TestTargetResolutionBecomesProvisionalWin(t *testing.T){
	...
	if !a.TargetResolved||a.Classification!=OutcomeSuccessfulCausalStep{t.Fatalf("assessment=%#v",a)}
	if k.Maturity!="PROVISIONAL_WIN"{t.Fatalf("knowledge=%#v",k)}
}
