...
func Assess(req Request) (Assessment, KnowledgeUpdate) {
	...
	before:=assertionMap(req.Before.Assertions);after:=assertionMap(req.After.Assertions)
	if req.TargetAssertion!=""{a.TargetResolved=!before[req.TargetAssertion].Pass&&after[req.TargetAssertion].Pass}
	for id,b:=range before{if b.Pass&&!after[id].Pass{a.Regressions=append(a.Regressions,id)}}
	...
	case OutcomeSuccessfulCausalStep:
		k.Action="PROMOTE_TO_PROVISIONAL_WIN";k.Maturity="PROVISIONAL_WIN";k.Preserve=append([]string(nil),req.ChangedModules...);k.Reason="target improved without material regression"
	case OutcomeRegression:
		k.Action="RECORD_REGRESSION";k.Avoid=append([]string(nil),req.ChangedModules...);k.Reason="candidate regressed previously passing assertions"
	default:
		k.Action="RECORD_FAILED_HYPOTHESIS";k.Avoid=append([]string(nil),req.ChangedModules...);k.Reason="target did not improve"
	}
	return a,k
}

type assertionState struct{Pass bool;Score float64}
func assertionMap(in []Assertion)map[string]assertionState{m:=map[string]assertionState{};for _,a:=range in{m[a.ID]=assertionState{Pass:a.Pass,Score:a.Score}};return m}
func causalConfidence(changed int)string{switch changed{case 0:return "UNKNOWN";case 1:return "HIGH";case 2:return "MEDIUM";default:return "LOW"}}
