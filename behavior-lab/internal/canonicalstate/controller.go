...
func BuildVerificationPlan(s State, maxBudget int) VerificationPlan {
	...
	controllerState := nextControllerState(ControllerReducing, s)
	p := VerificationPlan{
		Schema:            "tlaloc.verification-plan.r0",
		ControllerVersion: ControllerVersion,
		StateHash:         s.StateHash,
		Uncertainty:       s.Metrics.Uncertainty,
		Action:            controllerActions[controllerState],
	}
	...
	for _, conf := range s.Conflicts {
		...
		for _, id := range ids {
			for _, address := range evidenceByCandidate[id] {
				...
				seen[address] = struct{}{}
				ev = append(ev, address)
			}
		}
		sort.Strings(ev)
		p.Tasks = append(p.Tasks, VerificationTask{
			ID:            "verify:" + conf.ID,
			ConflictID:    conf.ID,
			Action:        "REOPEN_EXACT_EVIDENCE_AND_PROPOSE_VERDICT",
			Evidence:      ev,
			Priority:      1,
			ContextBudget: maxBudget,
		})
	}
