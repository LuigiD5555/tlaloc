...
func TestStoreIsIdempotentAndContentAddressed(t *testing.T) {
	...
	e.RecordedAt = "2026-02-01T00:00:00Z"
	added, b, err := s.Put(e)
	if err != nil {
		t.Fatal(err)
	}
	if added {
		t.Fatal("same evidence must deduplicate")
	}
	if a.EventID != b.EventID {
		t.Fatal("recorded_at must not change content id")
	}
...
func TestSummaryUsesRealFailuresBeforeSynthetic(t *testing.T) {
	...
	if s.RealModelObservations != 3 || s.SyntheticObservations != 1 {
		t.Fatalf("unexpected counts %#v", s)
	}
	if len(s.TopRealFailurePatterns) != 1 {
		t.Fatalf("patterns=%#v", s.TopRealFailurePatterns)
	}
	p := s.TopRealFailurePatterns[0]
	if p.Count != 2 || p.FailureCode != "T2_NOT_FOUND" || p.SuggestedTarget != "T2_NAVIGATION" {
		t.Fatalf("pattern=%#v", p)
	}
	if s.NextDebugTarget != "T2_NAVIGATION" {
		t.Fatalf("next=%s", s.NextDebugTarget)
	}
}

func TestCandidateOutcomeHistory(t *testing.T) {
	before := 0.4
	after := 0.8
	delta := 0.4
	...
	s := BuildSummary("mem", events)
	if s.ChangeAttempts != 1 || s.OutcomeLinks != 1 || len(s.CandidateOutcomes) != 1 {
		t.Fatalf("summary=%#v", s)
	}
	if s.CandidateOutcomes[0].MeanDelta != 0.4 {
		t.Fatalf("outcome=%#v", s.CandidateOutcomes[0])
	}
}
