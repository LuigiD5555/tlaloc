...
func TestDistillRepeatedTransitions(t *testing.T) {
	...
	if got.Automaton.Schema != AutomatonSchema {
		t.Fatalf("schema %s", got.Automaton.Schema)
	}
	if got.TemporalProgram.Schema != TemporalProgramSchema {
		t.Fatalf("temporal schema %s", got.TemporalProgram.Schema)
	}
	if got.TemporalProgram.Automaton.ID != got.Automaton.ID {
		t.Fatal("temporal program not bound to distilled automaton")
	}
	if got.TemporalProgram.MaxSteps != 4 {
		t.Fatalf("expected max_steps=4, got %d", got.TemporalProgram.MaxSteps)
	}
	...
	if len(got.Automaton.Cells) != 3 {
		t.Fatalf("expected 3 cells, got %d", len(got.Automaton.Cells))
	}
	if len(got.Automaton.Edges) < 2 {
		t.Fatalf("expected graph edges, got %#v", got.Automaton.Edges)
	}
}
...
func TestDistillOrderInvariantForSameExplicitSteps(t *testing.T) {
	...
	if ra.Automaton.SourceTraceSHA256 != rb.Automaton.SourceTraceSHA256 {
		t.Fatal("canonical trace digest drift")
	}
	if ra.Metrics.UniqueRules != rb.Metrics.UniqueRules {
		t.Fatal("rule count drift")
	}
	if ra.TemporalProgram.MaxSteps != rb.TemporalProgram.MaxSteps {
		t.Fatal("temporal horizon drift")
	}
}
...
func TestDistillRejectsImplicitStep(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected invalid explicit step rejection")
	}
}
