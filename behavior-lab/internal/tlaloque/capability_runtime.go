...
// CapabilityDescriptor is the stable contract Tlaloc uses to reason about a
// worker. The implementation may be a tiny encoder, encoder-decoder, local
// model, deterministic program, or a composite swarm.
...
// Tlaloque interface. It has no orchestration or promotion authority.
type CapabilityWorker interface {
	Descriptor() CapabilityDescriptor
	Execute(context.Context, CapabilityRequest) (CapabilityResponse, error)
}

// SelectionRequest lets a plan pin a worker or request a domain-specific
// specialist. Without a domain hint, GENERAL workers are preferred so Tlaloc
// never silently routes a task into an unrelated specialist.
...
func (r *Registry) Select(req SelectionRequest) (CapabilityWorker, error) {
	...
	for _, w := range r.workers {
		...
		if scopeHint == ScopeSpecific {
			if d.Scope != ScopeSpecific || domainHint == "" || d.Domain != domainHint {
				continue
			}
		} else if domainHint != "" && d.Scope == ScopeSpecific && d.Domain != domainHint {
			continue
		}
		score := 0
		...
		if domainHint == "" && d.Scope == ScopeGeneral {
			score += 25
		}
