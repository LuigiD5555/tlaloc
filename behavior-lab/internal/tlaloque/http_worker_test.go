...
func TestHTTPWorkerKeepsProtocolBounded(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.TaskID != "task" || req.NodeID != "intent" { t.Fatalf("req=%+v", req) }
	...
	if resp.WorkerID != "intent-http" || string(resp.Output) != `{"intent":"SEARCH"}` { t.Fatalf("resp=%+v", resp) }
}
...
func TestRegistryRefusesSpecificWorkerWithoutDomainEvidence(t *testing.T) {
	...
	if err := r.Register(worker); err != nil { t.Fatal(err) }
