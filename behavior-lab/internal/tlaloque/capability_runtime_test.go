...
func (w testWorker) Execute(ctx context.Context, req CapabilityRequest) (CapabilityResponse, error) {
	if w.active != nil {
		...
		for {
			...
			if n <= p || atomic.CompareAndSwapInt32(w.peak, p, n) { break }
		}
		defer atomic.AddInt32(w.active, -1)
	}
	if w.delay > 0 {
		select { case <-time.After(w.delay): case <-ctx.Done(): return CapabilityResponse{}, ctx.Err() }
	}
	out := json.RawMessage(`{"ok":true}`)
	if w.fn != nil { out = w.fn(req) }
...
func TestRegistryPrefersGeneralWithoutDomainAndSpecificWithDomain(t *testing.T) {
	...
	if err := r.Register(general); err != nil { t.Fatal(err) }
	if err := r.Register(specific); err != nil { t.Fatal(err) }
	w, err := r.Select(SelectionRequest{Capability:"EXTRACT_ENTITY"})
	if err != nil { t.Fatal(err) }
	if w.Descriptor().ID != "entity-general" { t.Fatalf("got %s", w.Descriptor().ID) }
	w, err = r.Select(SelectionRequest{Capability:"EXTRACT_ENTITY", ScopeHint:ScopeSpecific, DomainHint:"CFDI"})
	if err != nil { t.Fatal(err) }
	if w.Descriptor().ID != "entity-cfdi" { t.Fatalf("got %s", w.Descriptor().ID) }
}
...
func TestSwarmRunsIndependentNodesInParallelAndPassesDependencyContext(t *testing.T) {
	...
	router.fn = func(req CapabilityRequest) json.RawMessage {
		if len(req.Context["intent"]) == 0 || len(req.Context["entity"]) == 0 { panic("missing dependency context") }
		return json.RawMessage(`{"route":"documents"}`)
	}
	for _, w := range []CapabilityWorker{intent, entity, router} { if err := r.Register(w); err != nil { t.Fatal(err) } }
	...
	if err != nil { t.Fatal(err) }
	if !report.Succeeded { t.Fatalf("report=%+v", report) }
	if report.ExecutedNodes != 3 { t.Fatalf("executed=%d", report.ExecutedNodes) }
	if report.PeakParallel < 2 || atomic.LoadInt32(&peak) < 2 { t.Fatalf("peak=%d workerPeak=%d", report.PeakParallel, peak) }
	if string(report.TerminalOutputs["route"]) != `{"route":"documents"}` { t.Fatalf("terminal=%s", report.TerminalOutputs["route"]) }
}
...
func TestSwarmRejectsCycle(t *testing.T) {
	...
	if err == nil { t.Fatal("expected cycle error") }
}
...
func TestCompositeWorkerActsAsOneCapability(t *testing.T) {
	...
	if err := r.Register(atomic); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if resp.WorkerID != "composite" || !json.Valid(resp.Output) { t.Fatalf("resp=%+v", resp) }
	if len(resp.Output) == 0 { t.Fatal(fmt.Errorf("empty output")) }
}
