...
func TestResolveGoalBuildsPinnedDependencyDAG(t *testing.T) {
	...
	if len(resolved.Plan.Nodes) != 3 {
		t.Fatalf("nodes=%d", len(resolved.Plan.Nodes))
	}
	var root *SwarmNode
	for i := range resolved.Plan.Nodes {
		if resolved.Plan.Nodes[i].WorkerID == "router" {
			root = &resolved.Plan.Nodes[i]
		}
	}
	if root == nil {
		t.Fatal("router node not selected")
	}
	if len(root.DependsOn) != 2 {
		t.Fatalf("deps=%v", root.DependsOn)
	}
	if resolved.Plan.MaxParallel != 2 {
		t.Fatalf("max_parallel=%d", resolved.Plan.MaxParallel)
	}
}
