...
func compatibilityPanel(events []learningmemory.Event, baseline string) []experimentpolicy.ModelCompatibilityRequirement {
	type state struct { pass bool; evidence []string }
	byModel:=map[string]state{}
	for _,e:=range events{
		if e.EventType!=learningmemory.EventObservation||e.EvidenceClass!=learningmemory.EvidenceRealModel||e.Pass==nil||strings.TrimSpace(e.ModelID)==""{continue}
		if baseline!=""&&!matchesBaseline(e,baseline){continue}
		s:=byModel[e.ModelID];s.pass=*e.Pass;if e.EventID!=""{s.evidence=append(s.evidence,e.EventID)};byModel[e.ModelID]=s
	}
	ids:=make([]string,0,len(byModel));for id:=range byModel{ids=append(ids,id)};sort.Strings(ids)
	out:=make([]experimentpolicy.ModelCompatibilityRequirement,0,len(ids))
	for _,id:=range ids{
		s:=byModel[id];mode:=experimentpolicy.ModelCompatibilityImproveToPass;if s.pass{mode=experimentpolicy.ModelCompatibilityPreservePass}
...
func matchesBaseline(e learningmemory.Event, baseline string) bool {
	if e.CandidateID==baseline||e.SpecimenID==baseline{return true}
	return e.SpecimenID!=""&&strings.HasPrefix(e.SpecimenID,baseline+"-")
}

func panelModelIDs(panel []experimentpolicy.ModelCompatibilityRequirement) []string { out:=make([]string,0,len(panel));for _,p:=range panel{if p.ModelID!=""{out=append(out,p.ModelID)}};sort.Strings(out);return out }
func attachPanel(c experimentpolicy.CandidateManifest,panel []experimentpolicy.ModelCompatibilityRequirement) experimentpolicy.CandidateManifest { c.CompatibilityPanel=append([]experimentpolicy.ModelCompatibilityRequirement(nil),panel...);return c }
func uniqueStrings(in []string)[]string{set:=map[string]bool{};for _,v:=range in{if v!=""{set[v]=true}};out:=make([]string,0,len(set));for v:=range set{out=append(out,v)};sort.Strings(out);return out}
