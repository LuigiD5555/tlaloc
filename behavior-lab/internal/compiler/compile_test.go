...
func TestCompileUsesBehaviorSpecWithoutHardcodedProfileLaw(t *testing.T) {
	...
	p := Render(ir)
	for _, want := range []string{"TLALOC BEHAVIOR COMPILED PROMPT", "generic.example", "CUSTOM_RULE", "CUSTOM_INVARIANT", "Return one JSON object."} {
		if !strings.Contains(p, want) {
			t.Fatalf("missing %q", want)
		}
	}
	if strings.Contains(p, "Only an explicit OBSERVE") {
		t.Fatal("compiler leaked Origami-specific authority law into a generic profile")
	}
}
