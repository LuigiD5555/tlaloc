...
func TestExternalObservationIsSeparateFromWorkingConfiguration(t *testing.T) {
	...
	if reg.ModelID != "deepseek-vl2" || reg.Family != ModelFamilyDeepSeek { t.Fatalf("bad registry identity: %#v", reg) }
	if len(reg.Observations) != 1 { t.Fatalf("observations=%d want 1", len(reg.Observations)) }
	if reg.Observations[0].Outcome != "PARTIAL" { t.Fatalf("outcome=%q", reg.Observations[0].Outcome) }
}
