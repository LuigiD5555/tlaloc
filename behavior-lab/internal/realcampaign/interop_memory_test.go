...
func TestRecordWorkingConfigurationKeepsPerModelSuccessHistory(t *testing.T) {
	...
	if _, err := RecordWorkingConfiguration(root, second); err != nil {
		t.Fatal(err)
	}
	...
	if reg.ModelID != "lfm2-vl-1.6b" || reg.Family != ModelFamilyLiquidAI {
		t.Fatalf("bad registry identity: %#v", reg)
	}
	if len(reg.Configurations) != 1 {
		t.Fatalf("configurations=%d want 1", len(reg.Configurations))
	}
	cfg := reg.Configurations[0]
	if cfg.SuccessCount != 2 {
		t.Fatalf("success_count=%d want 2", cfg.SuccessCount)
	}
	if len(cfg.Evidence) != 2 {
		t.Fatalf("evidence=%d want 2", len(cfg.Evidence))
	}
	if cfg.Evidence[1].MeanNativeScore != 0.75 {
		t.Fatalf("native score=%v", cfg.Evidence[1].MeanNativeScore)
	}
}
...
func TestRecordWorkingConfigurationSeparatesExactModels(t *testing.T) {
	...
	p3, err := RecordWorkingConfiguration(root, q3)
	if err != nil {
		t.Fatal(err)
	}
	p7, err := RecordWorkingConfiguration(root, q7)
	if err != nil {
		t.Fatal(err)
	}
	if p3 == p7 {
		t.Fatal("different exact model ids must have separate registries")
	}
}
