...
func Evaluate(t Trial) Result {
	...
	for _, h := range t.Hops {
		...
		if hr.UndeclaredExternalCodecDependency || hr.SemanticToExactEscalation { hr.Pass = false }
		if readOK { readPass++ }
		if writeOK { writePass++ }
		if hr.UndeclaredExternalCodecDependency { result.UndeclaredCodecDependencyCount++ }
		if hr.SemanticToExactEscalation { result.SemanticToExactEscalationCount++ }
		if !hr.Pass { result.Pass = false }

		drift := hr.ReadSemanticDrift
		if t.Mode == ModeWrite { drift = hr.WriteSemanticDrift }
		if t.Mode == ModeRoundTrip || t.Mode == ModeMultiHop { drift = max(hr.ReadSemanticDrift, hr.WriteSemanticDrift) }
		driftSum += drift
		result.Hops = append(result.Hops, hr)
		if !emptyState(h.Reencoded) { prev = h.Reencoded } else { prev = h.Decoded }
	}
	...
	if len(t.Hops) > 0 {
		...
		if t.Mode == ModeRead { result.FinalSemanticDrift = semanticDrift(t.Source, last.Decoded) } else { result.FinalSemanticDrift = semanticDrift(t.Source, last.Reencoded) }
	}
	if t.Mode == ModeMultiHop && len(t.Hops) < 2 { result.Pass = false }
	return result
}
...
func semanticDrift(a, b SemanticState) float64 {
	...
	inter := 0
	for k := range as { if bs[k] { inter++ } }
	union := len(as)
	for k := range bs { if !as[k] { union++ } }
	if union == 0 { return 0 }
...
func inventedRate(want, got SemanticState) float64 {
	...
	if len(g) == 0 { return 0 }
	invented := 0
	for k := range g { if !w[k] { invented++ } }
	return float64(invented) / float64(len(g))
}

func atomSet(s SemanticState) map[string]bool {
	out := map[string]bool{}
	for k := range setStrings(s.Entities) { out["E|"+k] = true }
	for k := range setRelations(s.Relations) { out["R|"+k] = true }
	for k := range setRelations(s.Hierarchy) { out["H|"+k] = true }
	for k := range setStrings(s.Evidence) { out["V|"+k] = true }
	for k := range setStrings(s.Uncertainty) { out["U|"+k] = true }
	return out
}

func setStrings(values []string) map[string]bool {
	out := map[string]bool{}
	for _, v := range values { if n := normalize(v); n != "" { out[n] = true } }
	return out
}
...
func ratio(want, got map[string]bool) float64 {
	if len(want) == 0 { return 1 }
	matched := 0
	for k := range want { if got[k] { matched++ } }
...
func containsAny(text string, phrases []string) bool {
	for _, p := range phrases { if strings.Contains(text, p) { return true } }
	return false
}
...
func CanonicalAtoms(s SemanticState) []string {
	m := atomSet(s); out := make([]string, 0, len(m))
