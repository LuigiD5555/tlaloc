...
func TestPerfectMultiHopPasses(t *testing.T) {
	...
	if !res.Pass { t.Fatalf("perfect trial failed: %+v", res) }
	if res.FinalSemanticDrift != 0 || res.MeanSemanticDrift != 0 { t.Fatalf("unexpected drift: %+v", res) }
	if res.CrossModelReadSuccess != 1 || res.CrossModelWriteSuccess != 1 { t.Fatalf("success rate drift: %+v", res) }
}

func TestInventedRelationFails(t *testing.T) {
	s := sampleState(); bad := sampleState()
	...
	if res.Pass { t.Fatal("invented relation should fail") }
	if res.Hops[0].InventedReadFactRate == 0 || res.Hops[0].InventedWriteFactRate == 0 { t.Fatalf("invention not measured: %+v", res.Hops[0]) }
}
...
func TestUndeclaredExternalDecoderFails(t *testing.T) {
	...
	if res.Pass { t.Fatal("external codec dependency should fail") }
	if !res.Hops[0].UndeclaredExternalCodecDependency { t.Fatal("dependency violation not detected") }
}
...
func TestSemanticToExactEscalationFails(t *testing.T) {
	...
	if res.Pass { t.Fatal("semantic-to-exact escalation should fail") }
	if !res.Hops[0].SemanticToExactEscalation { t.Fatal("exact escalation not detected") }
}

func TestMissingSemanticFactMeasuresDrift(t *testing.T) {
	s := sampleState(); partial := sampleState(); partial.Relations = partial.Relations[:1]
	res := Evaluate(Trial{ID:"loss",Mode:ModeRead,RequiredDecoder:"S2",Source:s,Hops:[]Hop{{ModelID:"m",DecoderCodec:"S2",Decoded:partial}}})
	if res.FinalSemanticDrift <= 0 { t.Fatalf("expected positive drift: %+v", res) }
	if res.Hops[0].ReadPreservation.Relations >= 1 { t.Fatalf("relation loss not measured: %+v", res.Hops[0]) }
}
