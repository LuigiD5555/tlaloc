...
func TestCompilePreservesRequiredModulesAndUsesMinText(t *testing.T){
	...
	if len(out.Modules)!=2{t.Fatalf("modules=%d %#v",len(out.Modules),out.Modules)}
	if out.Modules[0].ID!="BOOT"||out.Modules[1].ID!="TEMPORAL"{t.Fatalf("order=%#v",out.Modules)}
	if out.Chars>24{t.Fatalf("chars=%d",out.Chars)}
}
