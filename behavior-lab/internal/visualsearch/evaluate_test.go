...
func TestInterestingPatternWithoutRealModelsCannotPromote(t *testing.T) {
	...
	if eval.PromotionCandidate {
		t.Fatalf("visual pattern promoted without real-model evidence: %+v", eval)
	}
}
...
func TestFalseExactBlocksAnyAestheticCandidate(t *testing.T) {
	...
	if eval.PromotionCandidate {
		t.Fatalf("FALSE_EXACT regression promoted: %+v", eval)
	}
}
...
func TestNativeIndexFailureBlocksCandidate(t *testing.T) {
	...
	m := improvedMetrics()
	m.NativeIndexRecoveryRate = .50
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if eval.PromotionCandidate {
		t.Fatalf("candidate that fails T2 index recovery promoted: %+v", eval)
	}
}
...
func TestMechanicalDependencyBlocksCandidate(t *testing.T) {
	...
	m := improvedMetrics()
	m.MechanicalDependencyViolations = 1
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if eval.PromotionCandidate {
		t.Fatalf("semantic navigation requiring mechanical decoder promoted: %+v", eval)
	}
}
...
func TestUnverifiedByteClaimsBlockCandidate(t *testing.T) {
	...
	m := improvedMetrics()
	m.UnverifiedMechanicalClaims = 2
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if eval.PromotionCandidate {
		t.Fatalf("candidate with unverified mechanical claims promoted: %+v", eval)
	}
}
...
func TestSemanticRoundtripRegressionBlocksCandidateEvenWithBetterPerception(t *testing.T) {
	...
	m := improvedMetrics()
	m.SemanticRoundtripRate = .99
	m.BootProbePassRate = 1
	m.TransportPassRate = 1
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if eval.PromotionCandidate {
		t.Fatalf("semantic regression promoted: %+v", eval)
	}
}
...
func TestMutationMustRemainExperimentalBeforePromotion(t *testing.T) {
	...
	if err == nil {
		t.Fatal("candidate claimed canonical status before Origami promotion")
	}
}
...
func TestTournamentRanksOnlyGatePassingCandidates(t *testing.T) {
	...
	badM := improvedMetrics()
	badM.UnknownViolations = 1
...
func TestMoireCandidateNeedsMeasuredReveal(t *testing.T) {
	...
	m := improvedMetrics()
	m.PerceptualRevealRate = .70
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if eval.PromotionCandidate {
		t.Fatalf("moire candidate promoted without reliable reveal: %+v", eval)
	}
}
...
func TestStereoCandidateCanWinWhenRevealAndSemanticsHold(t *testing.T) {
	...
	m := improvedMetrics()
	m.PerceptualRevealRate = .98
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if !eval.PromotionCandidate {
		t.Fatalf("evidence-backed depth candidate should be recommendable: %+v", eval)
	}
}
...
func TestOptimizationCanComeFromSmallerFasterFewerSteps(t *testing.T) {
	...
	m.NativeIndexRecoveryRate = .98
	m.NativeSemanticAnswerRate = .95
	eval, err := Evaluate(c, baselineMetrics(), Evidence{CandidateID: c.ID, Metrics: m}, DefaultPolicy())
	if err != nil {
		t.Fatal(err)
	}
	if !eval.PromotionCandidate {
		t.Fatalf("measured size/speed/decode improvement should count: %+v", eval)
	}
}
