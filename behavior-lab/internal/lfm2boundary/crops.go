...
func CropNearestPNG(data []byte, spec CropSpec) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decode carrier PNG: %w", err)
	}
	b := img.Bounds()
	if b.Dx() <= 0 || b.Dy() <= 0 {
		return nil, fmt.Errorf("empty carrier image")
	}
	...
	if r.Empty() {
		return nil, fmt.Errorf("crop %s is empty", spec.ID)
	}
	scale := spec.Scale
	if scale <= 0 {
		scale = 1
...
func WriteDeclaredCrops(carrierPath, outDir string) (map[string]string, error) {
	...
	for _, spec := range DeclaredCrops() {
		crop, err := CropNearestPNG(data, spec)
		if err != nil {
			return nil, err
		}
		path := filepath.Join(outDir, spec.ID+".png")
		if err := os.WriteFile(path, crop, 0o600); err != nil {
			return nil, err
		}
		paths[spec.ID] = path
	}
