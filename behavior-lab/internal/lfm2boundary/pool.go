...
func (w PooledProcessWorker) Execute(ctx context.Context, req tlaloque.CapabilityRequest) (tlaloque.CapabilityResponse, error) {
	role, ok := RoleFromNodeID(req.NodeID)
	if !ok {
		return tlaloque.CapabilityResponse{}, fmt.Errorf("cannot map node %q to a specialist responsibility", req.NodeID)
	}
