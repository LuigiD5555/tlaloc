...
func TestSimulateObservedRulesUsesSynchronousSnapshots(t *testing.T) {
	...
	states, err := SimulateObservedRules(cells,rules); if err!=nil{t.Fatal(err)}
	if states["A"]!="DONE"||states["B"]!="DONE"||states["C"]!="ACTIVE"{t.Fatalf("states=%v",states)}
}
...
func TestSimulateObservedRulesRejectsConflictingWrites(t *testing.T) {
	...
	if _,err:=SimulateObservedRules(cells,rules);err==nil{t.Fatal("expected conflicting write error")}
}

func TestStructuredObservationKeepsMalformedEvidenceForUnknown(t *testing.T){raw:=StructuredObservation(RoleCells,"not json");if !json.Valid(raw){t.Fatalf("not valid JSON: %s",raw)};if roleContract(RoleCells)(raw){t.Fatal("malformed evidence unexpectedly satisfied contract")}}

func TestSynthesizeBlackboardResponsesUsesTwoThirdsQuorumAndPassesTemporalBenchmark(t *testing.T) {
	root:=blackboard.New(t.TempDir());run:="r"
