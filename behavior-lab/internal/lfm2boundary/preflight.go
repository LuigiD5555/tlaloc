...
func Preflight(ctx context.Context, endpoint, model string) (PreflightResult, error) {
	...
	if err != nil {
		return PreflightResult{}, fmt.Errorf("LM Studio preflight: %w", err)
	}
	...
	if resp.StatusCode/100 != 2 {
		return PreflightResult{}, fmt.Errorf("LM Studio preflight status %s: %s", resp.Status, strings.TrimSpace(string(raw)))
	}
	var listing struct {
		Models []ListedModel `json:"models"`
	}
	if err := json.Unmarshal(raw, &listing); err != nil {
		return PreflightResult{}, fmt.Errorf("decode LM Studio model list: %w", err)
	}
	for _, m := range listing.Models {
		if m.Key != RequiredModel {
			continue
		}
		if m.Type != "llm" {
			return PreflightResult{}, fmt.Errorf("%s is not an LLM", RequiredModel)
		}
		if !m.Capabilities.Vision {
			return PreflightResult{}, fmt.Errorf("%s does not report vision capability", RequiredModel)
		}
		quant := ""
		if m.Quantization != nil {
			quant = strings.ToUpper(strings.TrimSpace(m.Quantization.Name))
		}
		if quant != "F16" {
			return PreflightResult{}, fmt.Errorf("%s must be the F16 variant, got %q", RequiredModel, quant)
		}
		for _, inst := range m.LoadedInstances {
			if inst.ID != RequiredModel {
				continue
			}
			if inst.Config.ContextLength != RequiredContext {
				return PreflightResult{}, fmt.Errorf("%s context must be %d, got %d", RequiredModel, RequiredContext, inst.Config.ContextLength)
			}
