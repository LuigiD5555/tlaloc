...
func Run(ctx context.Context, cfg Config) (Summary, error) {
	...
	for id, path := range map[string]string{"base": cfg.CarrierPath, "candidate": cfg.CandidatePath} {
		s, err := inspectSpecimen(id, path); if err != nil { return Summary{}, err }
...
func visualNodes(condition string, replicas, population int) ([]tlaloque.SwarmNode, []string) {
	...
	if strings.EqualFold(condition, "BLACKBOARD_CROPPED") {
		for _, role := range SpecialistRoles { for replica := 1; replica <= replicas; replica++ { add(fmt.Sprintf("%s-r%02d", strings.ToUpper(role), replica)) } }
		return nodes, deps
	}
	for _, q := range temporalbench.CanonicalQuestions() { for replica := 1; replica <= replicas; replica++ { add(fmt.Sprintf("%s-r%02d", q.ID, replica)) } }
	return nodes, deps
}

func expectedVisualResponses(condition string, replicas int) int {
	if strings.EqualFold(condition, "BLACKBOARD_CROPPED") { return len(SpecialistRoles) * replicas }
...
func runCell(ctx context.Context, cfg Config, store blackboard.Store, specimen SpecimenInfo, crops map[string]string, condition string, population, parallel int) (Metric, tlaloque.SwarmReport, error) {
	...
	for i := 0; i < population; i++ {
		...
		if err := registry.Register(w); err != nil { return Metric{RunID:runID}, tlaloque.SwarmReport{}, err }
	}
	consolidator := ConsolidatorWorker(cfg.WorkerBinary, cfg.Timeout)
	if err := registry.Register(consolidator); err != nil { return Metric{RunID:runID}, tlaloque.SwarmReport{}, err }
...
func measure(runID string, specimen SpecimenInfo, condition string, population, parallel, replicas int, report tlaloque.SwarmReport) Metric {
	...
	for _, n := range report.Nodes {
		if n.Error != "" {
			...
			if strings.Contains(low, "http") || strings.Contains(low, "target status") || strings.Contains(low, "connection") || strings.Contains(low, "eof") { m.HTTPErrorCount++ }
		}
		if n.Capability == VisualCapability && n.Error == "" {
			var out VisualOutput
			if err := json.Unmarshal(n.Output, &out); err == nil { m.ValidResponses++; m.PromptTokens += out.PromptTokens; m.CompletionTokens += out.CompletionTokens; latencies = append(latencies, n.DurationMS) }
		}
	...
	for _, q := range temporalbench.CanonicalQuestions() { if text, ok := consolidated.Responses[q.ID]; ok { responses = append(responses, temporalbench.Response{QuestionID:q.ID, Text:text}) } }
	...
	m.Score=result.OverallScore; m.TTemporal=result.TemporalReasoning; m.XExactness=result.ExactHonesty; m.InventedExactClaims=result.InventedExactClaims
...
func markUseful(metrics []Metric) {
	type key struct{ s, c string; p int }
	serial := map[key]Metric{}
	for _, m := range metrics { if m.MaxParallel == 1 { serial[key{m.SpecimenID,m.Condition,m.Population}] = m } }
	for i := range metrics {
		m := &metrics[i]; base, ok := serial[key{m.SpecimenID,m.Condition,m.Population}]
		if !ok || m.ErrorCount != 0 || m.ValidResponseRate != 1 || base.ErrorCount != 0 || base.ValidResponseRate != 1 { continue }
		if m.Score + 1.0/9.0 + 1e-9 < base.Score { continue }
		if base.LatencyP95MS > 0 && m.LatencyP95MS > 2*base.LatencyP95MS { continue }
		m.Useful = true
	}
}
...
func summarize(metrics []Metric, pops, pars []int) Summary {
	...
	for _, p := range pops {
		best := 0
		for _, c := range pars { seen, all := false, true; for _, m := range metrics { if m.Population==p && m.MaxParallel==c { seen=true; if !m.Useful { all=false; break } } }; if seen && all && c > best { best=c } }
		s.BoundaryByPopulation[strconv.Itoa(p)] = best; if best > s.MaxUsefulParallel { s.MaxUsefulParallel=best }
	}
...
func inspectSpecimen(id, path string) (SpecimenInfo, error) {
	b, err := os.ReadFile(path); if err != nil { return SpecimenInfo{}, err }
	sum := sha256.Sum256(b)
	cfg, err := png.DecodeConfig(bytes.NewReader(b)); if err != nil { return SpecimenInfo{}, fmt.Errorf("%s: %w", path, err) }
