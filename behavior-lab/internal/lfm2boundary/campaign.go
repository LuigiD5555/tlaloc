...
func Run(ctx context.Context, cfg Config) (Summary, error) {
	if cfg.Endpoint == "" {
		cfg.Endpoint = "http://127.0.0.1:1234/v1"
	}
	if cfg.Model == "" {
		cfg.Model = RequiredModel
	}
	if cfg.Replicas <= 0 {
		cfg.Replicas = 3
	}
	...
	if len(cfg.Populations) == 0 {
		cfg.Populations = []int{1, 2, 4, 8, 16}
	}
	if len(cfg.Parallelisms) == 0 {
		cfg.Parallelisms = []int{1, 2, 4}
	}
	if cfg.OutputDir == "" || cfg.WorkerBinary == "" || cfg.CarrierPath == "" || cfg.CandidatePath == "" {
		return Summary{}, fmt.Errorf("output dir, worker binary, carrier and candidate are required")
	}
	if strings.TrimSpace(cfg.R4Prompt) == "" {
		return Summary{}, fmt.Errorf("Master Prompt R4 is required for R4_ASSISTED")
	}
	for _, n := range append(append([]int{}, cfg.Populations...), cfg.Parallelisms...) {
		if n <= 0 {
			return Summary{}, fmt.Errorf("population and parallelism values must be positive")
		}
	}

	preflight, err := Preflight(ctx, cfg.Endpoint, cfg.Model)
	if err != nil {
		return Summary{}, err
	}
	if err := os.MkdirAll(filepath.Join(cfg.OutputDir, "local"), 0o700); err != nil {
		return Summary{}, err
	}
	...
	for _, s := range specimens {
		paths, err := WriteDeclaredCrops(s.Path, filepath.Join(cfg.OutputDir, "local", "crops", s.ID))
		if err != nil {
			return Summary{}, err
		}
	...
	if err := writeJSON(filepath.Join(cfg.OutputDir, "manifest.json"), manifest); err != nil {
		return Summary{}, err
	}
	if err := writeHashes(cfg, specimens); err != nil {
		return Summary{}, err
	}
		...
		for _, condition := range DefaultConditions {
			for _, population := range cfg.Populations {
				for _, parallel := range cfg.Parallelisms {
					metric, report, _ := runCell(ctx, cfg, store, specimen, cropPaths[specimen.ID], condition, population, parallel)
	...
	markUseful(metrics)
	if err := writeMetricsCSV(filepath.Join(cfg.OutputDir, "metrics.csv"), metrics); err != nil {
		return Summary{}, err
	}
...
func visualNodes(condition string, replicas, population int) ([]tlaloque.SwarmNode, []string) {
	nodes := []tlaloque.SwarmNode{}
	deps := []string{}
	idx := 0
	add := func(id string) {
		workerID := fmt.Sprintf("lfm2-slot-%02d", idx%population)
		nodes = append(nodes, tlaloque.SwarmNode{ID: id, Capability: VisualCapability, WorkerID: workerID})
		deps = append(deps, id)
		idx++
	}
	if strings.EqualFold(condition, "BLACKBOARD_CROPPED") {
		for _, role := range SpecialistRoles {
			for replica := 1; replica <= replicas; replica++ {
				add(fmt.Sprintf("%s-r%02d", strings.ToUpper(role), replica))
			}
		}
		return nodes, deps
	}
	for _, q := range temporalbench.CanonicalQuestions() {
		for replica := 1; replica <= replicas; replica++ {
			add(fmt.Sprintf("%s-r%02d", q.ID, replica))
		}
	}
	return nodes, deps
}

func expectedVisualResponses(condition string, replicas int) int {
	if strings.EqualFold(condition, "BLACKBOARD_CROPPED") {
		return len(SpecialistRoles) * replicas
	}
	return len(temporalbench.CanonicalQuestions()) * replicas
}
...
func runCell(ctx context.Context, cfg Config, store blackboard.Store, specimen SpecimenInfo, crops map[string]string, condition string, population, parallel int) (Metric, tlaloque.SwarmReport, error) {
	...
	for i := 0; i < population; i++ {
		w := PooledProcessWorker{ID: fmt.Sprintf("lfm2-slot-%02d", i), Binary: cfg.WorkerBinary, Timeout: cfg.Timeout}
		if err := registry.Register(w); err != nil {
			return Metric{RunID: runID}, tlaloque.SwarmReport{}, err
		}
	}
	consolidator := ConsolidatorWorker(cfg.WorkerBinary, cfg.Timeout)
	if err := registry.Register(consolidator); err != nil {
		return Metric{RunID: runID}, tlaloque.SwarmReport{}, err
	}
	nodes, deps := visualNodes(condition, cfg.Replicas, population)
	...
	task := VisualTask{Endpoint: cfg.Endpoint, Model: cfg.Model, ImagePath: specimen.Path, Crops: crops, Condition: condition, R4Prompt: cfg.R4Prompt, MaxTokens: cfg.MaxTokens}
...
func measure(runID string, specimen SpecimenInfo, condition string, population, parallel, replicas int, report tlaloque.SwarmReport) Metric {
	m := Metric{RunID: runID, SpecimenID: specimen.ID, Condition: condition, Population: population, MaxParallel: parallel, Replicas: replicas, ExpectedResponses: expectedVisualResponses(condition, replicas), PeakParallel: report.PeakParallel, DurationMS: report.DurationMS}
	latencies := []int64{}
	var consolidated ConsolidatedOutput
	for _, n := range report.Nodes {
		if n.Error != "" {
			m.ErrorCount++
			low := strings.ToLower(n.Error)
			if strings.Contains(low, "http") || strings.Contains(low, "target status") || strings.Contains(low, "connection") || strings.Contains(low, "eof") {
				m.HTTPErrorCount++
			}
		}
		if n.Capability == VisualCapability && n.Error == "" {
			...
			if err := json.Unmarshal(n.Output, &out); err == nil {
				m.ValidResponses++
				m.PromptTokens += out.PromptTokens
				m.CompletionTokens += out.CompletionTokens
				latencies = append(latencies, n.DurationMS)
			}
		}
		if n.NodeID == "consolidate" && n.Error == "" {
			_ = json.Unmarshal(n.Output, &consolidated)
		}
	}
	if m.ExpectedResponses > 0 {
		m.ValidResponseRate = float64(m.ValidResponses) / float64(m.ExpectedResponses)
	}
	m.LatencyP50MS = percentile(latencies, 0.50)
	m.LatencyP95MS = percentile(latencies, 0.95)
	if report.DurationMS > 0 {
		m.ThroughputPerSecond = float64(m.ValidResponses) / (float64(report.DurationMS) / 1000)
	}
	responses := []temporalbench.Response{}
	for _, q := range temporalbench.CanonicalQuestions() {
		if text, ok := consolidated.Responses[q.ID]; ok {
			responses = append(responses, temporalbench.Response{QuestionID: q.ID, Text: text})
		}
	}
	...
	result := temporalbench.EvaluateTrial(trial)
	m.Score = result.OverallScore
	m.TTemporal = result.TemporalReasoning
	m.XExactness = result.ExactHonesty
	m.InventedExactClaims = result.InventedExactClaims
	for _, l := range result.Layers {
		switch l.Layer {
		case "P_PERCEPTION":
			m.PPerception = l.Score
		case "R_PROTOCOL":
			m.RProtocol = l.Score
		case "S_SEMANTIC":
			m.SSemantic = l.Score
...
func markUseful(metrics []Metric) {
	...
	serial := map[key]Metric{}
	for _, m := range metrics {
		if m.MaxParallel == 1 {
			serial[key{m.SpecimenID, m.Condition, m.Population}] = m
		}
	}
	for i := range metrics {
		m := &metrics[i]
		base, ok := serial[key{m.SpecimenID, m.Condition, m.Population}]
		if !ok || m.ErrorCount != 0 || m.ValidResponseRate != 1 || base.ErrorCount != 0 || base.ValidResponseRate != 1 {
			continue
		}
		if m.Score+1.0/9.0+1e-9 < base.Score {
			continue
		}
		if base.LatencyP95MS > 0 && m.LatencyP95MS > 2*base.LatencyP95MS {
			continue
		}
		m.Useful = true
	}
...
func summarize(metrics []Metric, pops, pars []int) Summary {
	...
	for _, p := range pops {
		...
		for _, c := range pars {
			seen, all := false, true
			for _, m := range metrics {
				if m.Population == p && m.MaxParallel == c {
					seen = true
					if !m.Useful {
						all = false
						break
		...
		s.BoundaryByPopulation[strconv.Itoa(p)] = best
		if best > s.MaxUsefulParallel {
			s.MaxUsefulParallel = best
		}
	}
	for _, m := range metrics {
		if m.SpecimenID == "candidate" && m.Condition == "BLACKBOARD_CROPPED" && m.Useful && m.Score >= 6.0/9.0 && m.TTemporal > 0 && m.InventedExactClaims == 0 {
			s.MilestoneSuccess = true
			break
		}
	}
	return s
...
func inspectSpecimen(id, path string) (SpecimenInfo, error) {
	b, err := os.ReadFile(path)
	if err != nil {
		return SpecimenInfo{}, err
	}
	sum := sha256.Sum256(b)
	cfg, err := png.DecodeConfig(bytes.NewReader(b))
	if err != nil {
		return SpecimenInfo{}, fmt.Errorf("%s: %w", path, err)
	}
	return SpecimenInfo{ID: id, Path: path, SHA256: hex.EncodeToString(sum[:]), PNGBytes: len(b), Width: cfg.Width, Height: cfg.Height}, nil
}
...
func percentile(in []int64, q float64) int64 {
	...
	idx := int(float64(len(x)-1)*q + 0.999999)
	if idx < 0 {
		idx = 0
	}
	if idx >= len(x) {
...
func writeHashes(cfg Config, specimens []SpecimenInfo) error {
	h := map[string]string{}
	for _, s := range specimens {
		h[s.ID+"_png_sha256"] = s.SHA256
	}
	sum := sha256.Sum256([]byte(cfg.R4Prompt))
	h["master_prompt_r4_sha256"] = hex.EncodeToString(sum[:])
	return writeJSON(filepath.Join(cfg.OutputDir, "hashes.json"), h)
}
func writeMetricsCSV(path string, metrics []Metric) error {
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		return err
	}
