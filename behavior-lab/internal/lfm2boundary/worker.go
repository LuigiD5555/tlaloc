...
func ResponsibilityForQuestion(qid string) (string, bool) {
	...
	case "Q0", "Q6", "Q8":
		return RoleRosetta, true
	case "Q1", "Q2":
		return RoleCells, true
	case "Q3", "Q4":
		return RoleTransitions, true
	case "Q5", "Q7":
		return RoleTimeline, true
	default:
		return "", false
	}
}
...
func question(id string) (string, bool) {
	for _, q := range temporalbench.CanonicalQuestions() {
		if q.ID == strings.ToUpper(strings.TrimSpace(id)) {
			return q.Text, true
		}
	}
	return "", false
...
func RunVisualSpecialist(ctx context.Context, role string, req tlaloque.CapabilityRequest) (tlaloque.CapabilityResponse, error) {
	...
	if err := json.Unmarshal(req.Input, &task); err != nil {
		return tlaloque.CapabilityResponse{}, fmt.Errorf("decode visual task: %w", err)
	}
	condition := strings.ToUpper(strings.TrimSpace(task.Condition))
	resolvedRole, ok := RoleFromNodeID(req.NodeID)
	if !ok || !strings.EqualFold(resolvedRole, role) {
		return tlaloque.CapabilityResponse{}, fmt.Errorf("responsibility %s cannot serve node %s", role, req.NodeID)
	}
	...
	if condition == "BLACKBOARD_CROPPED" {
		...
		if strings.TrimSpace(imagePath) == "" {
			return tlaloque.CapabilityResponse{}, fmt.Errorf("missing declared crop %s for %s", cropID, role)
		}
		var err error
		...
		if !ok || !strings.EqualFold(wantRole, role) {
			return tlaloque.CapabilityResponse{}, fmt.Errorf("responsibility %s cannot answer %s", role, qid)
		}
		var found bool
		prompt, found = question(qid)
		if !found {
			return tlaloque.CapabilityResponse{}, fmt.Errorf("unknown benchmark question %q", qid)
		}
		observationKey = qid
		switch condition {
		...
		case "R4_ASSISTED":
			...
			if strings.TrimSpace(system) == "" {
				return tlaloque.CapabilityResponse{}, fmt.Errorf("R4_ASSISTED requires Master Prompt R4")
			}
		default:
...
func RunConsolidator(_ context.Context, req tlaloque.CapabilityRequest) (tlaloque.CapabilityResponse, error) {
	if req.Blackboard == nil {
		return tlaloque.CapabilityResponse{}, fmt.Errorf("consolidator requires blackboard snapshot")
	}
	var task VisualTask
	if err := json.Unmarshal(req.Input, &task); err != nil {
		return tlaloque.CapabilityResponse{}, fmt.Errorf("decode consolidator task: %w", err)
	}
	...
	if strings.EqualFold(task.Condition, "BLACKBOARD_CROPPED") {
		...
		for _, q := range temporalbench.CanonicalQuestions() {
			c, conErr := blackboard.Consolidate(*req.Blackboard, q.ID, nil)
			if conErr != nil {
				return tlaloque.CapabilityResponse{}, conErr
			}
			out.Consensus[q.ID] = c.Status
			answer := "UNKNOWN"
			if c.Status == blackboard.ConsensusConfirmed && len(c.Value) > 0 {
				if unmarshalErr := json.Unmarshal(c.Value, &answer); unmarshalErr != nil {
					return tlaloque.CapabilityResponse{}, unmarshalErr
				}
			}
	...
	for _, q := range temporalbench.CanonicalQuestions() {
		...
		if status == "" {
			...
			if answer != "UNKNOWN" {
				status = blackboard.ConsensusConfirmed
			}
		}
