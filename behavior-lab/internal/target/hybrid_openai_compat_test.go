...
func (m *mockExecutor) Execute(_ context.Context, name string, arguments json.RawMessage) (string, error) {
	m.calls++
	if name != "origami_follow" { return "", fmt.Errorf("unexpected tool %s", name) }
	var args struct {
...
func TestCompleteHybridSendsImageExecutesToolAndReturnsAnswer(t *testing.T) {
	requests := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok || len(messages) < 2 { t.Fatalf("unexpected messages: %#v", body["messages"]) }
		if requests == 1 {
			if _, ok := body["tools"].([]any); !ok { t.Fatalf("tools missing from first request: %#v", body) }
		...
		for _, raw := range messages {
			...
			if msg["role"] == "tool" {
				...
				if msg["tool_call_id"] != "call-1" { t.Fatalf("wrong tool_call_id: %#v", msg) }
			}
		}
		if !foundToolResult { t.Fatalf("second request missing tool result: %#v", messages) }
		w.Header().Set("Content-Type", "application/json")
	...
	if !strings.Contains(result.Answer, "AMBER-10593") { t.Fatalf("unexpected answer: %s", result.Answer) }
	if result.PromptTokensReported != 230 || result.CompletionTokensReported != 35 {
		t.Fatalf("unexpected usage: %+v", result)
	}
	if result.ToolOutputBytes == 0 || result.ToolOutputTokenEq == 0 { t.Fatalf("missing tool cost: %+v", result) }
}
