...
func (c OpenAICompat) CompleteHybrid(ctx context.Context, input HybridInput) (HybridResult, error) {
	if c.Model == "" { return HybridResult{}, fmt.Errorf("model is required") }
	if input.Executor == nil { return HybridResult{}, fmt.Errorf("tool executor is required") }
	if len(input.ImagePNG) == 0 { return HybridResult{}, fmt.Errorf("carrier image is required") }
	if input.MaxTurns <= 0 { input.MaxTurns = 16 }
	if len(input.Tools) == 0 { return HybridResult{}, fmt.Errorf("at least one declared tool is required") }
	...
	for turn := 0; turn < input.MaxTurns; turn++ {
		...
		if len(response.Choices) == 0 { return result, fmt.Errorf("target returned no choices") }
		choice := response.Choices[0]
		content := ""
		if choice.Message.Content != nil { content = strings.TrimSpace(*choice.Message.Content) }
		result.Turns = append(result.Turns, HybridTurn{Index: turn, ToolCalls: choice.Message.ToolCalls, Content: content})

		if len(choice.Message.ToolCalls) == 0 {
			if content == "" { return result, fmt.Errorf("target returned neither content nor tool calls") }
		...
		for _, call := range choice.Message.ToolCalls {
			if call.ID == "" || call.Function.Name == "" { return result, fmt.Errorf("invalid tool call") }
			args := json.RawMessage(call.Function.Arguments)
			if len(args) == 0 { args = json.RawMessage(`{}`) }
			var probe any
			if err := json.Unmarshal(args, &probe); err != nil { return result, fmt.Errorf("tool %s arguments: %w", call.Function.Name, err) }
			toolResult, err := input.Executor.Execute(ctx, call.Function.Name, args)
			if err != nil { return result, fmt.Errorf("execute tool %s: %w", call.Function.Name, err) }
			result.ToolCalls++
...
func (c OpenAICompat) hybridChat(ctx context.Context, messages []map[string]any, tools []ToolDefinition) (hybridChatResponse, error) {
	...
	if base == "" { base = "http://127.0.0.1:1234/v1" }
	client := c.Client
	...
	if err != nil { return hybridChatResponse{}, err }
	...
	if err != nil { return hybridChatResponse{}, err }
	if resp.StatusCode/100 != 2 { return hybridChatResponse{}, fmt.Errorf("target status %s: %s", resp.Status, string(b)) }
	var out hybridChatResponse
	if err := json.Unmarshal(b, &out); err != nil { return hybridChatResponse{}, err }
	return out, nil
}
