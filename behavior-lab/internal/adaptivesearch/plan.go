...
func BuildPlan(root string, events []learningmemory.Event) Plan {
	...
	for _, e := range events {
		if isRealFailure(e) { plan.RealFailureEvents++ }
	}
	for i, p := range summary.TopRealFailurePatterns {
		...
		if i == 0 { cp := focus; plan.PrimaryPattern = &cp }
	}
	...
	for _, k := range supportedKinds { raw[k] = 0 }
	if plan.Adaptive {
		for _, p := range plan.FailurePatterns {
			...
			for rank, kind := range targetMutationKinds(p.SuggestedTarget) {
				...
				f := 0.25
				if rank < len(factor) { f = factor[rank] }
				raw[kind] += base * f
			}
		}
	} else {
		for _, k := range supportedKinds { raw[k] = 1 }
	}

	history := historicalSignals(events)
	plan.HistoricalSignals = history
	for _, h := range history {
		if raw[h.MutationKind] <= 0 { continue }
	...
	sort.Slice(priorities, func(i,j int) bool {
		if priorities[i].Weight == priorities[j].Weight { return priorities[i].Kind < priorities[j].Kind }
		return priorities[i].Weight > priorities[j].Weight
	})
	for i := range priorities { priorities[i].Rank = i+1 }
...
func Prioritize(plan Plan, candidates []visualsearch.Candidate, limit int) Queue {
	...
	for _, p := range plan.MutationPriorities { weights[p.Kind] = p.Weight }
	...
	for _, c := range candidates {
		...
		score := 0.0
		if len(kinds)>0 { score=sum/float64(len(kinds)) }
		if len(c.Mutations)>1 { score -= 0.02*float64(len(c.Mutations)-1) }
		if score<0 { score=0 }
	...
	sort.Slice(items,func(i,j int)bool{if items[i].PriorityScore==items[j].PriorityScore{return items[i].CandidateID<items[j].CandidateID};return items[i].PriorityScore>items[j].PriorityScore})
	if limit>0&&limit<len(items){items=items[:limit]}
...
func ChangeAttemptEvents(queue Queue, candidates []visualsearch.Candidate) []learningmemory.Event {
	if !queue.Plan.Adaptive || len(queue.Plan.ParentEvidenceIDs)==0 { return nil }
	byID:=map[string]visualsearch.Candidate{};for _,c:=range candidates{byID[c.ID]=c}
	out:=[]learningmemory.Event{}
	for _,item:=range queue.CandidateOrder{
		c,ok:=byID[item.CandidateID];if !ok{continue}
		tags:=[]string{"adaptive-search","target:"+queue.Plan.NextDebugTarget}
		seen:=map[string]bool{}
		for _,m:=range c.Mutations{tag:="mutation:"+string(m.Kind);if !seen[tag]{tags=append(tags,tag);seen[tag]=true}}
		sort.Strings(tags)
...
func targetMutationKinds(target string) []visualsearch.MutationKind {
	...
	case "VISUAL_SIGNAL": return []visualsearch.MutationKind{visualsearch.MutationPrimitive,visualsearch.MutationRedundancy,visualsearch.MutationLayout,visualsearch.MutationColorUsage}
	case "BOOT": return []visualsearch.MutationKind{visualsearch.MutationPrompt,visualsearch.MutationLayout,visualsearch.MutationRedundancy,visualsearch.MutationPrimitive}
	case "ROSETTA": return []visualsearch.MutationKind{visualsearch.MutationPrompt,visualsearch.MutationLayout,visualsearch.MutationRedundancy,visualsearch.MutationPrimitive}
	case "CODEC_REGISTRY": return []visualsearch.MutationKind{visualsearch.MutationPrompt,visualsearch.MutationLayout,visualsearch.MutationChannelRole,visualsearch.MutationRedundancy}
	case "CAPABILITY_FALLBACK": return []visualsearch.MutationKind{visualsearch.MutationPrompt,visualsearch.MutationChannelRole,visualsearch.MutationRedundancy}
	case "T2_NAVIGATION": return []visualsearch.MutationKind{visualsearch.MutationLayout,visualsearch.MutationPrompt,visualsearch.MutationRedundancy,visualsearch.MutationChannelRole}
	case "SEMANTIC_LAYOUT": return []visualsearch.MutationKind{visualsearch.MutationLayout,visualsearch.MutationPrimitive,visualsearch.MutationRedundancy,visualsearch.MutationChannelRole}
	case "TEMPORAL_GRAMMAR": return []visualsearch.MutationKind{visualsearch.MutationTemporalStructure,visualsearch.MutationPrompt,visualsearch.MutationPrimitive,visualsearch.MutationChannelRole}
	case "TEMPORAL_ROUTING": return []visualsearch.MutationKind{visualsearch.MutationTemporalStructure,visualsearch.MutationLayout,visualsearch.MutationRedundancy,visualsearch.MutationPrompt}
	case "CAPABILITY_PROFILE": return []visualsearch.MutationKind{visualsearch.MutationPrompt,visualsearch.MutationChannelRole,visualsearch.MutationRedundancy}
	case "ROSETTA_TO_T2_ROUTE": return []visualsearch.MutationKind{visualsearch.MutationLayout,visualsearch.MutationPrompt,visualsearch.MutationRedundancy,visualsearch.MutationChannelRole}
	case "TEMPORAL_PROGRAM": return []visualsearch.MutationKind{visualsearch.MutationTemporalStructure,visualsearch.MutationLayout,visualsearch.MutationPrompt,visualsearch.MutationRedundancy}
	default:return []visualsearch.MutationKind{visualsearch.MutationPrompt,visualsearch.MutationLayout,visualsearch.MutationRedundancy,visualsearch.MutationPrimitive}
	}
}

func suggestion(target string, kind visualsearch.MutationKind) SuggestedMutation {
	t:=strings.ToUpper(strings.TrimSpace(target));if t==""{t="GENERAL_EXPLORATION"}
	targetField:=t
	value:="ISOLATED_EXPERIMENTAL_VARIANT"
	switch kind{
	case visualsearch.MutationLayout:
		if t=="T2_NAVIGATION"||t=="ROSETTA_TO_T2_ROUTE"{targetField="T1_TO_T2_ENTRY_ROUTE";value="EXPLICIT_DIRECTIONAL_ANCHOR"}else{value="LOCALIZE_FAILURE_TARGET_REGION"}
	case visualsearch.MutationPrompt:
		if t=="T2_NAVIGATION"||t=="ROSETTA_TO_T2_ROUTE"{targetField="ROSETTA.S2.READ_SUPERINDEX";value="DECLARE_T2_LOCATION_BEFORE_DECODE"}else{value="SHORT_EXPLICIT_PROTOCOL_INSTRUCTION"}
	case visualsearch.MutationRedundancy:
		if t=="T2_NAVIGATION"{targetField="T2_ENTRY_MARKER";value="REPEAT_AT_BOOT_AND_ROSETTA"}else{value="BOUNDED_REDUNDANT_ANCHOR"}
...
func historicalSignals(events []learningmemory.Event) []HistoricalSignal {
	...
	for _,e:=range events{
		...
		for _,tag:=range e.Tags{
			if !strings.HasPrefix(strings.ToLower(tag),"mutation:"){continue}
			...
			if candidateKinds[e.CandidateID]==nil{candidateKinds[e.CandidateID]=map[visualsearch.MutationKind]bool{}}
	...
	for _,e:=range events{
		if e.EventType!=learningmemory.EventOutcome||e.CandidateID==""||e.Delta==nil{continue}
		for kind:=range candidateKinds[e.CandidateID]{a:=byKind[kind];if a==nil{a=&acc{};byKind[kind]=a};a.n++;a.sum+=*e.Delta}
	}
	out:=[]HistoricalSignal{}
	for _,kind:=range supportedKinds{
		a:=byKind[kind];if a==nil||a.n==0{continue};mean:=a.sum/float64(a.n);adj:=clamp(mean,-0.25,0.25);out=append(out,HistoricalSignal{MutationKind:kind,Outcomes:a.n,MeanDelta:mean,Adjustment:adj})
	}
	return out
}
...
func parentEvidenceIDs(events []learningmemory.Event,p PatternFocus)[]string{
	...
	for _,e:=range events{
		...
		if stage==p.Stage&&failure==p.FailureCode&&layer==p.ScoreLayer&&e.EventID!=""{ids=append(ids,e.EventID)}
	}
	sort.Strings(ids);if len(ids)>20{ids=ids[:20]};return ids
}

func normalizedWithExploration(raw map[visualsearch.MutationKind]float64)map[visualsearch.MutationKind]float64{
	total:=0.0;for _,k:=range supportedKinds{if raw[k]>0{total+=raw[k]}}
	base:=map[visualsearch.MutationKind]float64{}
	if total<=0{for _,k:=range supportedKinds{base[k]=1/float64(len(supportedKinds))}}else{for _,k:=range supportedKinds{base[k]=math.Max(0,raw[k])/total}}
	floor:=explorationMass/float64(len(supportedKinds));for _,k:=range supportedKinds{base[k]=(1-explorationMass)*base[k]+floor}
	return base
}

func isRealFailure(e learningmemory.Event)bool{return e.EventType==learningmemory.EventObservation&&e.EvidenceClass==learningmemory.EvidenceRealModel&&e.Pass!=nil&&!*e.Pass}
func diversityFactor(models,specimens int)float64{return 1+0.1*float64(maxInt(0,models-1))+0.05*float64(maxInt(0,specimens-1))}
func containsKind(kinds []visualsearch.MutationKind,want visualsearch.MutationKind)bool{for _,k:=range kinds{if k==want{return true}};return false}
