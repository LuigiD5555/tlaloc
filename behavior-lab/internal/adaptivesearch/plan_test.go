...
func TestRealT2FailuresPrioritizeT2MutationFamilies(t *testing.T){
	...
	if !plan.Adaptive{t.Fatal("expected adaptive plan")}
	if plan.NextDebugTarget!="T2_NAVIGATION"{t.Fatalf("target=%s",plan.NextDebugTarget)}
	if len(plan.MutationPriorities)==0||plan.MutationPriorities[0].Kind!=visualsearch.MutationLayout{t.Fatalf("top priority=%+v",plan.MutationPriorities)}
	if len(plan.ParentEvidenceIDs)!=2{t.Fatalf("parents=%v",plan.ParentEvidenceIDs)}
	sum:=0.0;for _,p:=range plan.MutationPriorities{sum+=p.Weight;if p.Weight<=0{t.Fatalf("exploration floor missing for %s",p.Kind)}}
	if math.Abs(sum-1)>1e-9{t.Fatalf("weights sum=%f",sum)}
}
...
func TestSyntheticFailureDoesNotDriveAdaptiveFocus(t *testing.T){
	...
	plan:=BuildPlan("/memory",events)
	if plan.Adaptive{t.Fatal("synthetic evidence must not drive empirical adaptive focus")}
	if plan.NextDebugTarget!=""{t.Fatalf("unexpected target %q",plan.NextDebugTarget)}
}
...
func TestHistoricalSignalIsBounded(t *testing.T){
	...
	plan:=BuildPlan("/memory",events)
	if len(plan.HistoricalSignals)!=1{t.Fatalf("signals=%+v",plan.HistoricalSignals)}
	if plan.HistoricalSignals[0].Adjustment!=0.25{t.Fatalf("adjustment=%f",plan.HistoricalSignals[0].Adjustment)}
}
...
func TestPrioritizeQueueDoesNotClaimPromotion(t *testing.T){
	...
	if len(q.CandidateOrder)!=2||q.CandidateOrder[0].CandidateID!="layout"{t.Fatalf("order=%+v",q.CandidateOrder)}
	if q.Authority!="SEARCH_PRIORITY_ONLY_FINAL_VISUAL_TOURNAMENT_REMAINS_EVIDENCE_GATED"{t.Fatalf("authority=%s",q.Authority)}
}
...
func TestChangeAttemptsLinkBackToFailureEvidence(t *testing.T){
	...
	if len(attempts)!=1{t.Fatalf("attempts=%d",len(attempts))}
	if len(attempts[0].ParentEventIDs)!=1||attempts[0].ParentEventIDs[0]!="e1"{t.Fatalf("parents=%v",attempts[0].ParentEventIDs)}
	found:=false;for _,tag:=range attempts[0].Tags{if tag=="mutation:LAYOUT"{found=true}}
	if !found{t.Fatalf("tags=%v",attempts[0].Tags)}
}
