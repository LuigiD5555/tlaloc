...
func TestVisibleTextFidelityR6ExecutionCompliance(t *testing.T) {
	...
	for k, v := range executionComplianceVisibleFactsR1 {
		visibleFacts = append(visibleFacts, SemanticFact{Key: k, Value: v})
	}
	...
	if !report.Pass {
		t.Fatalf("expected R6 visible text to pass: %#v", report)
	}

	visible.Facts = visible.Facts[:len(visible.Facts)-1]
	report = CheckVisibleTextFidelity(candidate, semantic, visible)
	if report.Pass {
		t.Fatal("expected missing R6 directive to fail")
	}
}
