...
func TestCheckParityRejectsUnauthorizedRuleDrift(t *testing.T){
	c:=CandidateManifest{Schema:CandidateSchemaR1,ID:"c",ProgramSHA256:"p",Mutations:[]Mutation{{Kind:"PROMPT",Target:"EXECUTION_POLICY",Value:"EXECUTE_TO_STABLE"}}}
	...
	if r.Pass{t.Fatal("expected drift rejection")}
	if r.FailureCode!="UNAUTHORIZED_SEMANTIC_DRIFT"{t.Fatalf("failure=%q",r.FailureCode)}
}
...
func TestCheckParityAllowsDeclaredMutationOnly(t *testing.T){
	...
	r:=CheckParity(c,exp,act)
	if !r.Pass{t.Fatalf("unexpected reject: %#v",r)}
}
