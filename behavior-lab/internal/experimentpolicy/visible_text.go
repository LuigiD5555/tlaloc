...
func CheckVisibleTextFidelity(candidate CandidateManifest, semantic SemanticManifest, visible VisibleTextManifest) VisibleTextFidelityReport {
	...
	if visible.Schema != VisibleTextSchemaR1 {
		r.Pass = false
		r.FailureCode = "INVALID_VISIBLE_TEXT_MANIFEST"
		return r
	}
	if semantic.Schema != SemanticSchemaR1 {
		r.Pass = false
		r.FailureCode = "INVALID_SEMANTIC_MANIFEST"
		return r
	}
	if visible.ProgramSHA256 != semantic.ProgramSHA256 || (candidate.ProgramSHA256 != "" && visible.ProgramSHA256 != candidate.ProgramSHA256) {
		r.Differences = append(r.Differences, Difference{Key: "PROGRAM_SHA256", Expected: semantic.ProgramSHA256, Actual: visible.ProgramSHA256, Allowed: false})
	}
	sm := factMap(semantic.Facts)
	vm := factMap(visible.Facts)
	cellIDs := []string{}
	for k := range sm {
		if strings.HasPrefix(k, "VISIBLE_CELL_ID_") {
			cellIDs = append(cellIDs, strings.TrimPrefix(k, "VISIBLE_CELL_ID_"))
		}
	}
	sort.Strings(cellIDs)
	for _, id := range cellIDs {
		visibleID := sm["VISIBLE_CELL_ID_"+id]
		requireVisibleFact(&r, vm, "CELL."+id+".LABEL", "CELL "+visibleID)
	...
	if sm["RULE_ROLE_BINDING"] == visibleRuleRoleBindingR1 {
		...
		for i, id := range semanticRuleIDs(sm) {
			if i >= 4 {
				break
			}
			...
			if req == "" {
				...
				req = visibleRequires(req, sm)
			}
			target := sm["RULE."+id+".TARGET"]
			visibleTarget := sm["VISIBLE_CELL_ID_"+target]
			if visibleTarget == "" {
				visibleTarget = target
			}
			from := sm["RULE."+id+".FROM"]
			if from == "" {
				from = "*"
			}
			requireVisibleFact(&r, vm, "RULE."+id+".WHEN_TEXT", strings.ToUpper(id)+" WHEN "+req)
		...
		for i, id := range semanticRuleIDs(sm) {
			...
			want := fmt.Sprintf("IF %s => %s:%s>%s", req, visibleTarget, from, sm["RULE."+id+".TO"])
			requireVisibleFact(&r, vm, "RULE."+id+".TEXT", want)
		}
	}
	if sm["EXECUTION_POLICY"] == "EXECUTE_VISIBLE_RULES_TO_STABLE_R1" {
		requireVisibleFact(&r, vm, "EXECUTION_POLICY.TEXT", executeVisibleRulesToStableTextR1)
	}
	if sm["SYNCHRONOUS_EXECUTION_FIDELITY"] == freezeSelectApplyTogetherR1 {
		...
		keys := make([]string, 0, len(synchronousExecutionFidelityVisibleFactsR1))
		for k := range synchronousExecutionFidelityVisibleFactsR1 {
			keys = append(keys, k)
		}
		sort.Strings(keys)
		for _, k := range keys {
			requireVisibleFact(&r, vm, k, synchronousExecutionFidelityVisibleFactsR1[k])
		}
	} else if sm["EXECUTION_POLICY_COMPLIANCE"] == executeDontSummarizeToStableR1 {
		keys := make([]string, 0, len(executionComplianceVisibleFactsR1))
		for k := range executionComplianceVisibleFactsR1 {
			keys = append(keys, k)
		}
		sort.Strings(keys)
		for _, k := range keys {
			requireVisibleFact(&r, vm, k, executionComplianceVisibleFactsR1[k])
		}
	}
	if !r.Pass && r.FailureCode == "" {
		r.FailureCode = "VISIBLE_TEXT_FIDELITY_FAILED"
	}
	return r
}

func requireVisibleFact(r *VisibleTextFidelityReport, actual map[string]string, key, want string) {
	got, ok := actual[key]
	if !ok || got != want {
		r.Pass = false
		r.Differences = append(r.Differences, Difference{Key: key, Expected: want, Actual: got, Allowed: false})
	}
}
func semanticRuleIDs(facts map[string]string) []string {
	set := map[string]bool{}
	for k := range facts {
		if !strings.HasPrefix(k, "RULE.") || !strings.HasSuffix(k, ".TARGET") {
			continue
		}
		id := strings.TrimSuffix(strings.TrimPrefix(k, "RULE."), ".TARGET")
		if id != "" {
			set[id] = true
		}
	}
...
func visibleRequires(raw string, semantic map[string]string) string {
	...
	for i, p := range parts {
		...
		id := kv[0]
		visible := semantic["VISIBLE_CELL_ID_"+id]
		if visible == "" {
			visible = id
		}
		parts[i] = visible + "=" + kv[1]
	}
