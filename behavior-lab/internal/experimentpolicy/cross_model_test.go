...
func TestCrossModelCompatibilityPreservesWinsAndRequiresImprovement(t *testing.T) {
	...
	if !report.Pass {
		t.Fatalf("expected compatible panel, got %#v", report)
	}
}
...
func TestCrossModelCompatibilityRejectsDeepSeekRegression(t *testing.T) {
	...
	if report.Pass {
		t.Fatal("expected DeepSeek regression to reject candidate")
	}
	if report.FailureCode != "CROSS_MODEL_COMPATIBILITY_FAILED" {
		t.Fatalf("unexpected failure: %s", report.FailureCode)
	}
}
...
func TestCrossModelCompatibilityRequiresEveryPanelMember(t *testing.T) {
	...
	if report.Pass {
		t.Fatal("expected missing Qwen outcome to reject candidate")
	}
}
