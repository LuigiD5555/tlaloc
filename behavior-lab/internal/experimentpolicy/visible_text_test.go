...
func TestVisibleTextFidelityRejectsCellConfusion(t *testing.T) {
	...
	r := CheckVisibleTextFidelity(candidate, semantic, visible)
	if r.Pass || r.FailureCode != "VISIBLE_TEXT_FIDELITY_FAILED" {
		t.Fatalf("expected fidelity failure, got %#v", r)
	}
}
...
func TestRegressionPrecheckAllowsOnlyDeclaredIdentityChange(t *testing.T) {
	...
	r := CheckRegressionPreconditions(candidate, expected, build)
	if !r.Pass {
	...
	r = CheckRegressionPreconditions(candidate, expected, build)
	if r.Pass {
		t.Fatal("canonical rule regression must fail")
	}
}
