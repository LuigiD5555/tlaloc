...
func TestVisibleTextFidelityR7RequiresSynchronousProtocol(t *testing.T){
	...
	for k,v:=range synchronousExecutionFidelityVisibleFactsR1{facts=append(facts,SemanticFact{Key:k,Value:v})}
	...
	if !r.Pass{t.Fatalf("expected R7 visible protocol pass: %#v",r)}

	visible.Facts=visible.Facts[:len(visible.Facts)-1]
	r=CheckVisibleTextFidelity(candidate,semantic,visible)
	if r.Pass{t.Fatal("missing R7 protocol fact must fail")}
}
