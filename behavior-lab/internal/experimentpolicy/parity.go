...
func CheckParity(candidate CandidateManifest, expected, actual SemanticManifest) ParityReport {
	...
	if candidate.Schema != CandidateSchemaR1 {
		r.Pass = false
		r.FailureCode = "INVALID_CANDIDATE_MANIFEST"
		return r
	}
	...
	for _, k := range ordered {
		...
		if changed {
			if act[k] == want {
				if exp[k] != act[k] {
					r.Differences = append(r.Differences, Difference{Key: k, Expected: want, Actual: act[k], Allowed: true})
				}
				continue
			}
			r.Differences = append(r.Differences, Difference{Key: k, Expected: want, Actual: act[k], Allowed: false})
			continue
		}
		if exp[k] != act[k] {
			r.Differences = append(r.Differences, Difference{Key: k, Expected: exp[k], Actual: act[k], Allowed: false})
		}
	}
	...
	if !r.Pass {
		r.FailureCode = "UNAUTHORIZED_SEMANTIC_DRIFT"
	}
	return r
}

func ValidateBuild(candidate CandidateManifest, build BuildManifest) error {
	if build.Schema != BuildSchemaR1 {
		return fmt.Errorf("unexpected build schema %q", build.Schema)
	}
	if build.CandidateID != candidate.ID {
		return fmt.Errorf("candidate id mismatch")
	}
	if candidate.ProgramSHA256 != "" && build.ProgramSHA256 != candidate.ProgramSHA256 {
		return fmt.Errorf("program sha mismatch")
	}
	if candidate.PayloadSHA256 != "" && build.PayloadSHA256 != candidate.PayloadSHA256 {
		return fmt.Errorf("payload sha mismatch")
	}
	if build.ArtifactSHA256 == "" || build.ArtifactBytes <= 0 {
		return fmt.Errorf("artifact provenance incomplete")
	}
	return nil
}

func factMap(f []SemanticFact) map[string]string {
	m := map[string]string{}
	for _, x := range f {
		m[x.Key] = x.Value
	}
