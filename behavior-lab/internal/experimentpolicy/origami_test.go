...
func TestToOrigamiSpecMarksMutationsExperimental(t *testing.T) {
	...
	s, err := ToOrigamiSpec(c, "parent-sha")
	if err != nil {
		t.Fatal(err)
	}
	if s.Schema != OrigamiCandidateSpecSchemaR0 || s.ParentSHA256 != "parent-sha" {
		t.Fatalf("spec=%#v", s)
	}
	if len(s.Mutations) != 1 || !s.Mutations[0].Experimental {
		t.Fatalf("mutation=%#v", s.Mutations)
	}
}
