...
func TestContentIDStableAndAppendIdempotent(t *testing.T) {
	...
	id2, err := ContentID(e)
	if err != nil { t.Fatal(err) }
	if id1 != id2 { t.Fatalf("unstable ids: %s != %s", id1, id2) }
	added, stored, err := s.Append(e)
	if err != nil || !added { t.Fatalf("added=%v err=%v", added, err) }
	added, again, err := s.Append(e)
	if err != nil || added { t.Fatalf("second added=%v err=%v", added, err) }
	if stored.ID != again.ID { t.Fatalf("ids differ: %s %s", stored.ID, again.ID) }
}
...
func TestAppendPublishesOneAtomicEntryUnderConcurrency(t *testing.T) {
	...
	all, err := s.LoadAll()
	if err != nil { t.Fatal(err) }
	if len(all) != 1 { t.Fatalf("entries=%d want 1", len(all)) }
}
...
func TestSnapshotIsolatedByRunAndSortedByID(t *testing.T) {
	...
	} { if _, _, err := s.Append(e); err != nil { t.Fatal(err) } }
	snap, err := s.Snapshot("run-a")
	if err != nil { t.Fatal(err) }
	if len(snap.Entries) != 2 { t.Fatalf("entries=%d", len(snap.Entries)) }
	if snap.Entries[0].ID > snap.Entries[1].ID { t.Fatal("snapshot is not sorted by ID") }
	for _, e := range snap.Entries { if e.RunID != "run-a" { t.Fatalf("leaked run %s", e.RunID) } }
}
...
func TestConsolidateRequiresTwoThirdsAndPreservesConflicts(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if c.Status != ConsensusConfirmed || c.Votes != 2 || c.RequiredVotes != 2 { t.Fatalf("consensus=%+v", c) }
	if len(snap.Entries) != 3 { t.Fatalf("contradiction was lost: %d", len(snap.Entries)) }
}
...
func TestConsolidateTieOrContractViolationIsUnknown(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if c.Status != ConsensusUnknown { t.Fatalf("tie=%+v", c) }
	c, err = Consolidate(snap, "k", func(v json.RawMessage) bool { return string(v) == `"A"` })
	if err != nil { t.Fatal(err) }
	if c.Status != ConsensusUnknown { t.Fatalf("contract=%+v", c) }
}
