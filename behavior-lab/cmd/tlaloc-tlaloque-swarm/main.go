...
func goalFlags(name string, args []string) (string, tlaloque.CapabilityGoal, string, int, *flag.FlagSet) {
	...
	fs.Parse(args)
	if manifest == "" || goal.Capability == "" { die(fmt.Errorf("--manifest and --goal are required")) }
...
func plan(args []string) {
	...
	r, err := m.Registry(); die(err)
	resolved, err := r.ResolveGoal(goal, planID, maxParallel); die(err)
	write(resolved)
}
...
func run(ctx context.Context, args []string) {
	...
	if selectedPlan.ID == "" || len(selectedPlan.Nodes) == 0 { die(fmt.Errorf("manifest has no plan; provide --goal for automatic planning")) }
...
func usage() { fmt.Fprintln(os.Stderr, "usage: tlaloc-tlaloque-swarm <validate|catalog|plan|run|example> [flags]") }
