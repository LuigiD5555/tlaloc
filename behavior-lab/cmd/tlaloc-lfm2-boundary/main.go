...
func main() {
	...
	pars, err := parseInts(*parallelisms)
	if err != nil {
		fail(err)
	}
	if *carrier == "" || *candidate == "" {
		fail(fmt.Errorf("--carrier and --candidate are required"))
	}
	workerPath := *worker
	if workerPath == "" {
		...
		workerPath = filepath.Join(filepath.Dir(exe), "tlaloc-lfm2-worker")
	}
	r4Path := *r4
	if r4Path == "" {
		r4Path = defaultR4Path()
	}
	r4Body, err := os.ReadFile(r4Path)
	if err != nil {
		fail(fmt.Errorf("read Master Prompt R4 %s: %w", r4Path, err))
	}
	summary, err := lfm2boundary.Run(context.Background(), lfm2boundary.Config{Endpoint: *endpoint, Model: *model, CarrierPath: *carrier, CandidatePath: *candidate, R4Prompt: string(r4Body), WorkerBinary: workerPath, OutputDir: *out, Populations: pops, Parallelisms: pars, Replicas: *replicas, MaxTokens: *maxTokens, Timeout: *timeout})
	if err != nil {
		fail(err)
	}
...
func parseInts(s string) ([]int, error) {
	...
	for _, raw := range parts {
		...
		if err != nil || n <= 0 {
			return nil, fmt.Errorf("invalid positive integer %q", raw)
		}
		if !seen[n] {
			seen[n] = true
			out = append(out, n)
...
func defaultR4Path() string {
	data := strings.TrimSpace(os.Getenv("XDG_DATA_HOME"))
	if data == "" {
		home, _ := os.UserHomeDir()
		data = filepath.Join(home, ".local", "share")
	}
	return filepath.Join(data, "origami", "current", "generated", "MASTER_PROMPT.md")
}
func usage() {
	fmt.Fprintln(os.Stderr, "Usage: tlaloc-lfm2-boundary run --carrier BASE.png --candidate CANDIDATE.png [--endpoint http://127.0.0.1:1234/v1] [--model lfm2-vl-1.6b] [--populations 1,2,4,8,16] [--parallelisms 1,2,4] [--replicas 3] [--out DIR]")
}
