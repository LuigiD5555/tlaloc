...
func main() {
	...
	if err := json.Unmarshal(b, &trace); err != nil { fatal(err.Error()) }
	result, err := automata.Distill(trace); if err != nil { fatal(err.Error()) }
	var f *os.File
	if *out == "" || *out == "-" { f = os.Stdout } else { f, err = os.Create(*out); if err != nil { fatal(err.Error()) }; defer f.Close() }
	enc := json.NewEncoder(f); enc.SetIndent("", "  ")
	if err := enc.Encode(result); err != nil { fatal(err.Error()) }
}
