...
func main() {
	...
	flag.Parse()
	if *printDebug {
		fmt.Println(temporalbench.DiagnosticInstruction())
		return
	}
	if *printExample {
		fmt.Println(temporalbench.FormatDebugExample())
		return
	}
	body, err := read(*in)
	die(err)
	...
	die(dec.Decode(&campaign))
	if campaign.Schema != temporalbench.CampaignSchema {
		die(fmt.Errorf("unexpected schema %q", campaign.Schema))
	}
	if campaign.BenchmarkID == "" {
		die(fmt.Errorf("benchmark_id is required"))
	}
	...
	if result.RealEvidence && !*noMemory {
		events, err := learningmemory.ImportTemporalBenchmark(body, encoded, campaign, result, learningmemory.ImportOptions{OrigamiVersion: *origamiVersion, TlalocVersion: *tlalocVersion, CandidateID: *candidateID})
		die(err)
		if len(events) > 0 {
			store := learningmemory.New(*memoryRoot)
			_, _, _, err = store.PutAll(events)
			die(err)
		}
	}
	if *out == "-" {
